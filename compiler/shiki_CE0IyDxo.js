import { c as eQ } from "./micromark_CAniCe8D.js";
let b = class extends Error {
  constructor(A) {
    super(A), this.name = "ShikiError";
  }
}, ZA = class extends Error {
  constructor(A) {
    super(A), this.name = "ShikiError";
  }
};
function iQ() {
  return 2147483648;
}
function tQ() {
  return typeof performance < "u" ? performance.now() : Date.now();
}
const cQ = (B, A) => B + (A - B % A) % A;
async function nQ(B) {
  let A, Q;
  const g = {};
  function E(i) {
    Q = i, g.HEAPU8 = new Uint8Array(i), g.HEAPU32 = new Uint32Array(i);
  }
  function C(i, t, N) {
    g.HEAPU8.copyWithin(i, t, t + N);
  }
  function I(i) {
    try {
      return A.grow(i - Q.byteLength + 65535 >>> 16), E(A.buffer), 1;
    } catch {
    }
  }
  function w(i) {
    const t = g.HEAPU8.length;
    i = i >>> 0;
    const N = iQ();
    if (i > N)
      return !1;
    for (let M = 1; M <= 4; M *= 2) {
      let h = t * (1 + 0.2 / M);
      h = Math.min(h, i + 100663296);
      const a = Math.min(N, cQ(Math.max(i, h), 65536));
      if (I(a))
        return !0;
    }
    return !1;
  }
  const D = typeof TextDecoder < "u" ? new TextDecoder("utf8") : void 0;
  function o(i, t, N = 1024) {
    const M = t + N;
    let h = t;
    for (; i[h] && !(h >= M); )
      ++h;
    if (h - t > 16 && i.buffer && D)
      return D.decode(i.subarray(t, h));
    let a = "";
    for (; t < h; ) {
      let F = i[t++];
      if (!(F & 128)) {
        a += String.fromCharCode(F);
        continue;
      }
      const R = i[t++] & 63;
      if ((F & 224) === 192) {
        a += String.fromCharCode((F & 31) << 6 | R);
        continue;
      }
      const L = i[t++] & 63;
      if ((F & 240) === 224 ? F = (F & 15) << 12 | R << 6 | L : F = (F & 7) << 18 | R << 12 | L << 6 | i[t++] & 63, F < 65536)
        a += String.fromCharCode(F);
      else {
        const u = F - 65536;
        a += String.fromCharCode(55296 | u >> 10, 56320 | u & 1023);
      }
    }
    return a;
  }
  function s(i, t) {
    return i ? o(g.HEAPU8, i, t) : "";
  }
  const e = {
    emscripten_get_now: tQ,
    emscripten_memcpy_big: C,
    emscripten_resize_heap: w,
    fd_write: () => 0
  };
  async function n() {
    const t = await B({
      env: e,
      wasi_snapshot_preview1: e
    });
    A = t.memory, E(A.buffer), Object.assign(g, t), g.UTF8ToString = s;
  }
  return await n(), g;
}
var MQ = Object.defineProperty, aQ = (B, A, Q) => A in B ? MQ(B, A, { enumerable: !0, configurable: !0, writable: !0, value: Q }) : B[A] = Q, k = (B, A, Q) => (aQ(B, typeof A != "symbol" ? A + "" : A, Q), Q);
let U = null;
function hQ(B) {
  throw new ZA(B.UTF8ToString(B.getLastOnigError()));
}
class LA {
  constructor(A) {
    k(this, "utf16Length"), k(this, "utf8Length"), k(this, "utf16Value"), k(this, "utf8Value"), k(this, "utf16OffsetToUtf8"), k(this, "utf8OffsetToUtf16");
    const Q = A.length, g = LA._utf8ByteLength(A), E = g !== Q, C = E ? new Uint32Array(Q + 1) : null;
    E && (C[Q] = g);
    const I = E ? new Uint32Array(g + 1) : null;
    E && (I[g] = Q);
    const w = new Uint8Array(g);
    let D = 0;
    for (let o = 0; o < Q; o++) {
      const s = A.charCodeAt(o);
      let e = s, n = !1;
      if (s >= 55296 && s <= 56319 && o + 1 < Q) {
        const i = A.charCodeAt(o + 1);
        i >= 56320 && i <= 57343 && (e = (s - 55296 << 10) + 65536 | i - 56320, n = !0);
      }
      E && (C[o] = D, n && (C[o + 1] = D), e <= 127 ? I[D + 0] = o : e <= 2047 ? (I[D + 0] = o, I[D + 1] = o) : e <= 65535 ? (I[D + 0] = o, I[D + 1] = o, I[D + 2] = o) : (I[D + 0] = o, I[D + 1] = o, I[D + 2] = o, I[D + 3] = o)), e <= 127 ? w[D++] = e : e <= 2047 ? (w[D++] = 192 | (e & 1984) >>> 6, w[D++] = 128 | (e & 63) >>> 0) : e <= 65535 ? (w[D++] = 224 | (e & 61440) >>> 12, w[D++] = 128 | (e & 4032) >>> 6, w[D++] = 128 | (e & 63) >>> 0) : (w[D++] = 240 | (e & 1835008) >>> 18, w[D++] = 128 | (e & 258048) >>> 12, w[D++] = 128 | (e & 4032) >>> 6, w[D++] = 128 | (e & 63) >>> 0), n && o++;
    }
    this.utf16Length = Q, this.utf8Length = g, this.utf16Value = A, this.utf8Value = w, this.utf16OffsetToUtf8 = C, this.utf8OffsetToUtf16 = I;
  }
  static _utf8ByteLength(A) {
    let Q = 0;
    for (let g = 0, E = A.length; g < E; g++) {
      const C = A.charCodeAt(g);
      let I = C, w = !1;
      if (C >= 55296 && C <= 56319 && g + 1 < E) {
        const D = A.charCodeAt(g + 1);
        D >= 56320 && D <= 57343 && (I = (C - 55296 << 10) + 65536 | D - 56320, w = !0);
      }
      I <= 127 ? Q += 1 : I <= 2047 ? Q += 2 : I <= 65535 ? Q += 3 : Q += 4, w && g++;
    }
    return Q;
  }
  createString(A) {
    const Q = A.omalloc(this.utf8Length);
    return A.HEAPU8.set(this.utf8Value, Q), Q;
  }
}
const S = class {
  constructor(B) {
    if (k(this, "id", ++S.LAST_ID), k(this, "_onigBinding"), k(this, "content"), k(this, "utf16Length"), k(this, "utf8Length"), k(this, "utf16OffsetToUtf8"), k(this, "utf8OffsetToUtf16"), k(this, "ptr"), !U)
      throw new ZA("Must invoke loadWasm first.");
    this._onigBinding = U, this.content = B;
    const A = new LA(B);
    this.utf16Length = A.utf16Length, this.utf8Length = A.utf8Length, this.utf16OffsetToUtf8 = A.utf16OffsetToUtf8, this.utf8OffsetToUtf16 = A.utf8OffsetToUtf16, this.utf8Length < 1e4 && !S._sharedPtrInUse ? (S._sharedPtr || (S._sharedPtr = U.omalloc(1e4)), S._sharedPtrInUse = !0, U.HEAPU8.set(A.utf8Value, S._sharedPtr), this.ptr = S._sharedPtr) : this.ptr = A.createString(U);
  }
  convertUtf8OffsetToUtf16(B) {
    return this.utf8OffsetToUtf16 ? B < 0 ? 0 : B > this.utf8Length ? this.utf16Length : this.utf8OffsetToUtf16[B] : B;
  }
  convertUtf16OffsetToUtf8(B) {
    return this.utf16OffsetToUtf8 ? B < 0 ? 0 : B > this.utf16Length ? this.utf8Length : this.utf16OffsetToUtf8[B] : B;
  }
  dispose() {
    this.ptr === S._sharedPtr ? S._sharedPtrInUse = !1 : this._onigBinding.ofree(this.ptr);
  }
};
let EA = S;
k(EA, "LAST_ID", 0);
k(EA, "_sharedPtr", 0);
k(EA, "_sharedPtrInUse", !1);
class FQ {
  constructor(A) {
    if (k(this, "_onigBinding"), k(this, "_ptr"), !U)
      throw new ZA("Must invoke loadWasm first.");
    const Q = [], g = [];
    for (let w = 0, D = A.length; w < D; w++) {
      const o = new LA(A[w]);
      Q[w] = o.createString(U), g[w] = o.utf8Length;
    }
    const E = U.omalloc(4 * A.length);
    U.HEAPU32.set(Q, E / 4);
    const C = U.omalloc(4 * A.length);
    U.HEAPU32.set(g, C / 4);
    const I = U.createOnigScanner(E, C, A.length);
    for (let w = 0, D = A.length; w < D; w++)
      U.ofree(Q[w]);
    U.ofree(C), U.ofree(E), I === 0 && hQ(U), this._onigBinding = U, this._ptr = I;
  }
  dispose() {
    this._onigBinding.freeOnigScanner(this._ptr);
  }
  findNextMatchSync(A, Q, g) {
    let E = 0;
    if (typeof g == "number" && (E = g), typeof A == "string") {
      A = new EA(A);
      const C = this._findNextMatchSync(A, Q, !1, E);
      return A.dispose(), C;
    }
    return this._findNextMatchSync(A, Q, !1, E);
  }
  _findNextMatchSync(A, Q, g, E) {
    const C = this._onigBinding, I = C.findNextOnigScannerMatch(this._ptr, A.id, A.ptr, A.utf8Length, A.convertUtf16OffsetToUtf8(Q), E);
    if (I === 0)
      return null;
    const w = C.HEAPU32;
    let D = I / 4;
    const o = w[D++], s = w[D++], e = [];
    for (let n = 0; n < s; n++) {
      const i = A.convertUtf8OffsetToUtf16(w[D++]), t = A.convertUtf8OffsetToUtf16(w[D++]);
      e[n] = {
        start: i,
        end: t,
        length: t - i
      };
    }
    return {
      index: o,
      captureIndices: e
    };
  }
}
function GQ(B) {
  return typeof B.instantiator == "function";
}
function NQ(B) {
  return typeof B.default == "function";
}
function rQ(B) {
  return typeof B.data < "u";
}
function LQ(B) {
  return typeof Response < "u" && B instanceof Response;
}
function HQ(B) {
  return typeof ArrayBuffer < "u" && (B instanceof ArrayBuffer || ArrayBuffer.isView(B)) || typeof Buffer < "u" && Buffer.isBuffer?.(B) || typeof SharedArrayBuffer < "u" && B instanceof SharedArrayBuffer || typeof Uint32Array < "u" && B instanceof Uint32Array;
}
let DA;
function RQ(B) {
  if (DA)
    return DA;
  async function A() {
    U = await nQ(async (Q) => {
      let g = B;
      return g = await g, typeof g == "function" && (g = await g(Q)), typeof g == "function" && (g = await g(Q)), GQ(g) ? g = await g.instantiator(Q) : NQ(g) ? g = await g.default(Q) : (rQ(g) && (g = g.data), LQ(g) ? typeof WebAssembly.instantiateStreaming == "function" ? g = await kQ(g)(Q) : g = await KQ(g)(Q) : HQ(g) ? g = await UA(g)(Q) : g instanceof WebAssembly.Module ? g = await UA(g)(Q) : "default" in g && g.default instanceof WebAssembly.Module && (g = await UA(g.default)(Q))), "instance" in g && (g = g.instance), "exports" in g && (g = g.exports), g;
    });
  }
  return DA = A(), DA;
}
function UA(B) {
  return (A) => WebAssembly.instantiate(B, A);
}
function kQ(B) {
  return (A) => WebAssembly.instantiateStreaming(B, A);
}
function KQ(B) {
  return async (A) => {
    const Q = await B.arrayBuffer();
    return WebAssembly.instantiate(Q, A);
  };
}
let UQ;
function YQ() {
  return UQ;
}
async function lQ(B) {
  return B && await RQ(B), {
    createScanner(A) {
      return new FQ(A.map((Q) => typeof Q == "string" ? Q : Q.source));
    },
    createString(A) {
      return new EA(A);
    }
  };
}
function PQ(B) {
  return vA(B);
}
function vA(B) {
  return Array.isArray(B) ? yQ(B) : B instanceof RegExp ? B : typeof B == "object" ? JQ(B) : B;
}
function yQ(B) {
  let A = [];
  for (let Q = 0, g = B.length; Q < g; Q++)
    A[Q] = vA(B[Q]);
  return A;
}
function JQ(B) {
  let A = {};
  for (let Q in B)
    A[Q] = vA(B[Q]);
  return A;
}
function KB(B, ...A) {
  return A.forEach((Q) => {
    for (let g in Q)
      B[g] = Q[g];
  }), B;
}
function UB(B) {
  const A = ~B.lastIndexOf("/") || ~B.lastIndexOf("\\");
  return A === 0 ? B : ~A === B.length - 1 ? UB(B.substring(0, B.length - 1)) : B.substr(~A + 1);
}
var YA = /\$(\d+)|\${(\d+):\/(downcase|upcase)}/g, wA = class {
  static hasCaptures(B) {
    return B === null ? !1 : (YA.lastIndex = 0, YA.test(B));
  }
  static replaceCaptures(B, A, Q) {
    return B.replace(YA, (g, E, C, I) => {
      let w = Q[parseInt(E || C, 10)];
      if (w) {
        let D = A.substring(w.start, w.end);
        for (; D[0] === "."; )
          D = D.substring(1);
        switch (I) {
          case "downcase":
            return D.toLowerCase();
          case "upcase":
            return D.toUpperCase();
          default:
            return D;
        }
      } else
        return g;
    });
  }
};
function YB(B, A) {
  return B < A ? -1 : B > A ? 1 : 0;
}
function lB(B, A) {
  if (B === null && A === null)
    return 0;
  if (!B)
    return -1;
  if (!A)
    return 1;
  let Q = B.length, g = A.length;
  if (Q === g) {
    for (let E = 0; E < Q; E++) {
      let C = YB(B[E], A[E]);
      if (C !== 0)
        return C;
    }
    return 0;
  }
  return Q - g;
}
function CB(B) {
  return !!(/^#[0-9a-f]{6}$/i.test(B) || /^#[0-9a-f]{8}$/i.test(B) || /^#[0-9a-f]{3}$/i.test(B) || /^#[0-9a-f]{4}$/i.test(B));
}
function PB(B) {
  return B.replace(/[\-\\\{\}\*\+\?\|\^\$\.\,\[\]\(\)\#\s]/g, "\\$&");
}
var yB = class {
  constructor(B) {
    this.fn = B;
  }
  cache = /* @__PURE__ */ new Map();
  get(B) {
    if (this.cache.has(B))
      return this.cache.get(B);
    const A = this.fn(B);
    return this.cache.set(B, A), A;
  }
}, tA = class {
  constructor(B, A, Q) {
    this._colorMap = B, this._defaults = A, this._root = Q;
  }
  static createFromRawTheme(B, A) {
    return this.createFromParsedTheme(pQ(B), A);
  }
  static createFromParsedTheme(B, A) {
    return dQ(B, A);
  }
  _cachedMatchRoot = new yB(
    (B) => this._root.match(B)
  );
  getColorMap() {
    return this._colorMap.getColorMap();
  }
  getDefaults() {
    return this._defaults;
  }
  match(B) {
    if (B === null)
      return this._defaults;
    const A = B.scopeName, g = this._cachedMatchRoot.get(A).find(
      (E) => uQ(B.parent, E.parentScopes)
    );
    return g ? new JB(
      g.fontStyle,
      g.foreground,
      g.background
    ) : null;
  }
}, lA = class eA {
  constructor(A, Q) {
    this.parent = A, this.scopeName = Q;
  }
  static push(A, Q) {
    for (const g of Q)
      A = new eA(A, g);
    return A;
  }
  static from(...A) {
    let Q = null;
    for (let g = 0; g < A.length; g++)
      Q = new eA(Q, A[g]);
    return Q;
  }
  push(A) {
    return new eA(this, A);
  }
  getSegments() {
    let A = this;
    const Q = [];
    for (; A; )
      Q.push(A.scopeName), A = A.parent;
    return Q.reverse(), Q;
  }
  toString() {
    return this.getSegments().join(" ");
  }
  extends(A) {
    return this === A ? !0 : this.parent === null ? !1 : this.parent.extends(A);
  }
  getExtensionIfDefined(A) {
    const Q = [];
    let g = this;
    for (; g && g !== A; )
      Q.push(g.scopeName), g = g.parent;
    return g === A ? Q.reverse() : void 0;
  }
};
function uQ(B, A) {
  if (A.length === 0)
    return !0;
  for (let Q = 0; Q < A.length; Q++) {
    let g = A[Q], E = !1;
    if (g === ">") {
      if (Q === A.length - 1)
        return !1;
      g = A[++Q], E = !0;
    }
    for (; B && !SQ(B.scopeName, g); ) {
      if (E)
        return !1;
      B = B.parent;
    }
    if (!B)
      return !1;
    B = B.parent;
  }
  return !0;
}
function SQ(B, A) {
  return A === B || B.startsWith(A) && B[A.length] === ".";
}
var JB = class {
  constructor(B, A, Q) {
    this.fontStyle = B, this.foregroundId = A, this.backgroundId = Q;
  }
};
function pQ(B) {
  if (!B)
    return [];
  if (!B.settings || !Array.isArray(B.settings))
    return [];
  let A = B.settings, Q = [], g = 0;
  for (let E = 0, C = A.length; E < C; E++) {
    let I = A[E];
    if (!I.settings)
      continue;
    let w;
    if (typeof I.scope == "string") {
      let e = I.scope;
      e = e.replace(/^[,]+/, ""), e = e.replace(/[,]+$/, ""), w = e.split(",");
    } else Array.isArray(I.scope) ? w = I.scope : w = [""];
    let D = -1;
    if (typeof I.settings.fontStyle == "string") {
      D = 0;
      let e = I.settings.fontStyle.split(" ");
      for (let n = 0, i = e.length; n < i; n++)
        switch (e[n]) {
          case "italic":
            D = D | 1;
            break;
          case "bold":
            D = D | 2;
            break;
          case "underline":
            D = D | 4;
            break;
          case "strikethrough":
            D = D | 8;
            break;
        }
    }
    let o = null;
    typeof I.settings.foreground == "string" && CB(I.settings.foreground) && (o = I.settings.foreground);
    let s = null;
    typeof I.settings.background == "string" && CB(I.settings.background) && (s = I.settings.background);
    for (let e = 0, n = w.length; e < n; e++) {
      let t = w[e].trim().split(" "), N = t[t.length - 1], M = null;
      t.length > 1 && (M = t.slice(0, t.length - 1), M.reverse()), Q[g++] = new fQ(
        N,
        M,
        E,
        D,
        o,
        s
      );
    }
  }
  return Q;
}
var fQ = class {
  constructor(B, A, Q, g, E, C) {
    this.scope = B, this.parentScopes = A, this.index = Q, this.fontStyle = g, this.foreground = E, this.background = C;
  }
}, d = /* @__PURE__ */ ((B) => (B[B.NotSet = -1] = "NotSet", B[B.None = 0] = "None", B[B.Italic = 1] = "Italic", B[B.Bold = 2] = "Bold", B[B.Underline = 4] = "Underline", B[B.Strikethrough = 8] = "Strikethrough", B))(d || {});
function dQ(B, A) {
  B.sort((D, o) => {
    let s = YB(D.scope, o.scope);
    return s !== 0 || (s = lB(D.parentScopes, o.parentScopes), s !== 0) ? s : D.index - o.index;
  });
  let Q = 0, g = "#000000", E = "#ffffff";
  for (; B.length >= 1 && B[0].scope === ""; ) {
    let D = B.shift();
    D.fontStyle !== -1 && (Q = D.fontStyle), D.foreground !== null && (g = D.foreground), D.background !== null && (E = D.background);
  }
  let C = new OQ(A), I = new JB(Q, C.getId(g), C.getId(E)), w = new qQ(new pA(0, null, -1, 0, 0), []);
  for (let D = 0, o = B.length; D < o; D++) {
    let s = B[D];
    w.insert(0, s.scope, s.parentScopes, s.fontStyle, C.getId(s.foreground), C.getId(s.background));
  }
  return new tA(C, I, w);
}
var OQ = class {
  _isFrozen;
  _lastColorId;
  _id2color;
  _color2id;
  constructor(B) {
    if (this._lastColorId = 0, this._id2color = [], this._color2id = /* @__PURE__ */ Object.create(null), Array.isArray(B)) {
      this._isFrozen = !0;
      for (let A = 0, Q = B.length; A < Q; A++)
        this._color2id[B[A]] = A, this._id2color[A] = B[A];
    } else
      this._isFrozen = !1;
  }
  getId(B) {
    if (B === null)
      return 0;
    B = B.toUpperCase();
    let A = this._color2id[B];
    if (A)
      return A;
    if (this._isFrozen)
      throw new Error(`Missing color in color map - ${B}`);
    return A = ++this._lastColorId, this._color2id[B] = A, this._id2color[A] = B, A;
  }
  getColorMap() {
    return this._id2color.slice(0);
  }
}, xQ = Object.freeze([]), pA = class uB {
  scopeDepth;
  parentScopes;
  fontStyle;
  foreground;
  background;
  constructor(A, Q, g, E, C) {
    this.scopeDepth = A, this.parentScopes = Q || xQ, this.fontStyle = g, this.foreground = E, this.background = C;
  }
  clone() {
    return new uB(this.scopeDepth, this.parentScopes, this.fontStyle, this.foreground, this.background);
  }
  static cloneArr(A) {
    let Q = [];
    for (let g = 0, E = A.length; g < E; g++)
      Q[g] = A[g].clone();
    return Q;
  }
  acceptOverwrite(A, Q, g, E) {
    this.scopeDepth > A ? console.log("how did this happen?") : this.scopeDepth = A, Q !== -1 && (this.fontStyle = Q), g !== 0 && (this.foreground = g), E !== 0 && (this.background = E);
  }
}, qQ = class fA {
  constructor(A, Q = [], g = {}) {
    this._mainRule = A, this._children = g, this._rulesWithParentScopes = Q;
  }
  _rulesWithParentScopes;
  static _cmpBySpecificity(A, Q) {
    if (A.scopeDepth !== Q.scopeDepth)
      return Q.scopeDepth - A.scopeDepth;
    let g = 0, E = 0;
    for (; A.parentScopes[g] === ">" && g++, Q.parentScopes[E] === ">" && E++, !(g >= A.parentScopes.length || E >= Q.parentScopes.length); ) {
      const C = Q.parentScopes[E].length - A.parentScopes[g].length;
      if (C !== 0)
        return C;
      g++, E++;
    }
    return Q.parentScopes.length - A.parentScopes.length;
  }
  match(A) {
    if (A !== "") {
      let g = A.indexOf("."), E, C;
      if (g === -1 ? (E = A, C = "") : (E = A.substring(0, g), C = A.substring(g + 1)), this._children.hasOwnProperty(E))
        return this._children[E].match(C);
    }
    const Q = this._rulesWithParentScopes.concat(this._mainRule);
    return Q.sort(fA._cmpBySpecificity), Q;
  }
  insert(A, Q, g, E, C, I) {
    if (Q === "") {
      this._doInsertHere(A, g, E, C, I);
      return;
    }
    let w = Q.indexOf("."), D, o;
    w === -1 ? (D = Q, o = "") : (D = Q.substring(0, w), o = Q.substring(w + 1));
    let s;
    this._children.hasOwnProperty(D) ? s = this._children[D] : (s = new fA(this._mainRule.clone(), pA.cloneArr(this._rulesWithParentScopes)), this._children[D] = s), s.insert(A + 1, o, g, E, C, I);
  }
  _doInsertHere(A, Q, g, E, C) {
    if (Q === null) {
      this._mainRule.acceptOverwrite(A, g, E, C);
      return;
    }
    for (let I = 0, w = this._rulesWithParentScopes.length; I < w; I++) {
      let D = this._rulesWithParentScopes[I];
      if (lB(D.parentScopes, Q) === 0) {
        D.acceptOverwrite(A, g, E, C);
        return;
      }
    }
    g === -1 && (g = this._mainRule.fontStyle), E === 0 && (E = this._mainRule.foreground), C === 0 && (C = this._mainRule.background), this._rulesWithParentScopes.push(new pA(A, Q, g, E, C));
  }
}, v = class J {
  static toBinaryStr(A) {
    return A.toString(2).padStart(32, "0");
  }
  static print(A) {
    const Q = J.getLanguageId(A), g = J.getTokenType(A), E = J.getFontStyle(A), C = J.getForeground(A), I = J.getBackground(A);
    console.log({
      languageId: Q,
      tokenType: g,
      fontStyle: E,
      foreground: C,
      background: I
    });
  }
  static getLanguageId(A) {
    return (A & 255) >>> 0;
  }
  static getTokenType(A) {
    return (A & 768) >>> 8;
  }
  static containsBalancedBrackets(A) {
    return (A & 1024) !== 0;
  }
  static getFontStyle(A) {
    return (A & 30720) >>> 11;
  }
  static getForeground(A) {
    return (A & 16744448) >>> 15;
  }
  static getBackground(A) {
    return (A & 4278190080) >>> 24;
  }
  /**
   * Updates the fields in `metadata`.
   * A value of `0`, `NotSet` or `null` indicates that the corresponding field should be left as is.
   */
  static set(A, Q, g, E, C, I, w) {
    let D = J.getLanguageId(A), o = J.getTokenType(A), s = J.containsBalancedBrackets(A) ? 1 : 0, e = J.getFontStyle(A), n = J.getForeground(A), i = J.getBackground(A);
    return Q !== 0 && (D = Q), g !== 8 && (o = g), E !== null && (s = E ? 1 : 0), C !== -1 && (e = C), I !== 0 && (n = I), w !== 0 && (i = w), (D << 0 | o << 8 | s << 10 | e << 11 | n << 15 | i << 24) >>> 0;
  }
};
function cA(B, A) {
  const Q = [], g = mQ(B);
  let E = g.next();
  for (; E !== null; ) {
    let D = 0;
    if (E.length === 2 && E.charAt(1) === ":") {
      switch (E.charAt(0)) {
        case "R":
          D = 1;
          break;
        case "L":
          D = -1;
          break;
        default:
          console.log(`Unknown priority ${E} in scope selector`);
      }
      E = g.next();
    }
    let o = I();
    if (Q.push({ matcher: o, priority: D }), E !== ",")
      break;
    E = g.next();
  }
  return Q;
  function C() {
    if (E === "-") {
      E = g.next();
      const D = C();
      return (o) => !!D && !D(o);
    }
    if (E === "(") {
      E = g.next();
      const D = w();
      return E === ")" && (E = g.next()), D;
    }
    if (IB(E)) {
      const D = [];
      do
        D.push(E), E = g.next();
      while (IB(E));
      return (o) => A(D, o);
    }
    return null;
  }
  function I() {
    const D = [];
    let o = C();
    for (; o; )
      D.push(o), o = C();
    return (s) => D.every((e) => e(s));
  }
  function w() {
    const D = [];
    let o = I();
    for (; o && (D.push(o), E === "|" || E === ","); ) {
      do
        E = g.next();
      while (E === "|" || E === ",");
      o = I();
    }
    return (s) => D.some((e) => e(s));
  }
}
function IB(B) {
  return !!B && !!B.match(/[\w\.:]+/);
}
function mQ(B) {
  let A = /([LR]:|[\w\.:][\w\.:\-]*|[\,\|\-\(\)])/g, Q = A.exec(B);
  return {
    next: () => {
      if (!Q)
        return null;
      const g = Q[0];
      return Q = A.exec(B), g;
    }
  };
}
function SB(B) {
  typeof B.dispose == "function" && B.dispose();
}
var AA = class {
  constructor(B) {
    this.scopeName = B;
  }
  toKey() {
    return this.scopeName;
  }
}, bQ = class {
  constructor(B, A) {
    this.scopeName = B, this.ruleName = A;
  }
  toKey() {
    return `${this.scopeName}#${this.ruleName}`;
  }
}, TQ = class {
  _references = [];
  _seenReferenceKeys = /* @__PURE__ */ new Set();
  get references() {
    return this._references;
  }
  visitedRule = /* @__PURE__ */ new Set();
  add(B) {
    const A = B.toKey();
    this._seenReferenceKeys.has(A) || (this._seenReferenceKeys.add(A), this._references.push(B));
  }
}, WQ = class {
  constructor(B, A) {
    this.repo = B, this.initialScopeName = A, this.seenFullScopeRequests.add(this.initialScopeName), this.Q = [new AA(this.initialScopeName)];
  }
  seenFullScopeRequests = /* @__PURE__ */ new Set();
  seenPartialScopeRequests = /* @__PURE__ */ new Set();
  Q;
  processQueue() {
    const B = this.Q;
    this.Q = [];
    const A = new TQ();
    for (const Q of B)
      XQ(Q, this.initialScopeName, this.repo, A);
    for (const Q of A.references)
      if (Q instanceof AA) {
        if (this.seenFullScopeRequests.has(Q.scopeName))
          continue;
        this.seenFullScopeRequests.add(Q.scopeName), this.Q.push(Q);
      } else {
        if (this.seenFullScopeRequests.has(Q.scopeName) || this.seenPartialScopeRequests.has(Q.toKey()))
          continue;
        this.seenPartialScopeRequests.add(Q.toKey()), this.Q.push(Q);
      }
  }
};
function XQ(B, A, Q, g) {
  const E = Q.lookup(B.scopeName);
  if (!E) {
    if (B.scopeName === A)
      throw new Error(`No grammar provided for <${A}>`);
    return;
  }
  const C = Q.lookup(A);
  B instanceof AA ? iA({ baseGrammar: C, selfGrammar: E }, g) : dA(
    B.ruleName,
    { baseGrammar: C, selfGrammar: E, repository: E.repository },
    g
  );
  const I = Q.injections(B.scopeName);
  if (I)
    for (const w of I)
      g.add(new AA(w));
}
function dA(B, A, Q) {
  if (A.repository && A.repository[B]) {
    const g = A.repository[B];
    nA([g], A, Q);
  }
}
function iA(B, A) {
  B.selfGrammar.patterns && Array.isArray(B.selfGrammar.patterns) && nA(
    B.selfGrammar.patterns,
    { ...B, repository: B.selfGrammar.repository },
    A
  ), B.selfGrammar.injections && nA(
    Object.values(B.selfGrammar.injections),
    { ...B, repository: B.selfGrammar.repository },
    A
  );
}
function nA(B, A, Q) {
  for (const g of B) {
    if (Q.visitedRule.has(g))
      continue;
    Q.visitedRule.add(g);
    const E = g.repository ? KB({}, A.repository, g.repository) : A.repository;
    Array.isArray(g.patterns) && nA(g.patterns, { ...A, repository: E }, Q);
    const C = g.include;
    if (!C)
      continue;
    const I = pB(C);
    switch (I.kind) {
      case 0:
        iA({ ...A, selfGrammar: A.baseGrammar }, Q);
        break;
      case 1:
        iA(A, Q);
        break;
      case 2:
        dA(I.ruleName, { ...A, repository: E }, Q);
        break;
      case 3:
      case 4:
        const w = I.scopeName === A.selfGrammar.scopeName ? A.selfGrammar : I.scopeName === A.baseGrammar.scopeName ? A.baseGrammar : void 0;
        if (w) {
          const D = { baseGrammar: A.baseGrammar, selfGrammar: w, repository: E };
          I.kind === 4 ? dA(I.ruleName, D, Q) : iA(D, Q);
        } else
          I.kind === 4 ? Q.add(new bQ(I.scopeName, I.ruleName)) : Q.add(new AA(I.scopeName));
        break;
    }
  }
}
var ZQ = class {
  kind = 0;
}, vQ = class {
  kind = 1;
}, VQ = class {
  constructor(B) {
    this.ruleName = B;
  }
  kind = 2;
}, zQ = class {
  constructor(B) {
    this.scopeName = B;
  }
  kind = 3;
}, jQ = class {
  constructor(B, A) {
    this.scopeName = B, this.ruleName = A;
  }
  kind = 4;
};
function pB(B) {
  if (B === "$base")
    return new ZQ();
  if (B === "$self")
    return new vQ();
  const A = B.indexOf("#");
  if (A === -1)
    return new zQ(B);
  if (A === 0)
    return new VQ(B.substring(1));
  {
    const Q = B.substring(0, A), g = B.substring(A + 1);
    return new jQ(Q, g);
  }
}
var _Q = /\\(\d+)/, DB = /\\(\d+)/g, $Q = -1, fB = -2;
var CA = class {
  $location;
  id;
  _nameIsCapturing;
  _name;
  _contentNameIsCapturing;
  _contentName;
  constructor(B, A, Q, g) {
    this.$location = B, this.id = A, this._name = Q || null, this._nameIsCapturing = wA.hasCaptures(this._name), this._contentName = g || null, this._contentNameIsCapturing = wA.hasCaptures(this._contentName);
  }
  get debugName() {
    const B = this.$location ? `${UB(this.$location.filename)}:${this.$location.line}` : "unknown";
    return `${this.constructor.name}#${this.id} @ ${B}`;
  }
  getName(B, A) {
    return !this._nameIsCapturing || this._name === null || B === null || A === null ? this._name : wA.replaceCaptures(this._name, B, A);
  }
  getContentName(B, A) {
    return !this._contentNameIsCapturing || this._contentName === null ? this._contentName : wA.replaceCaptures(this._contentName, B, A);
  }
}, Ag = class extends CA {
  retokenizeCapturedWithRuleId;
  constructor(B, A, Q, g, E) {
    super(B, A, Q, g), this.retokenizeCapturedWithRuleId = E;
  }
  dispose() {
  }
  collectPatterns(B, A) {
    throw new Error("Not supported!");
  }
  compile(B, A) {
    throw new Error("Not supported!");
  }
  compileAG(B, A, Q, g) {
    throw new Error("Not supported!");
  }
}, Bg = class extends CA {
  _match;
  captures;
  _cachedCompiledPatterns;
  constructor(B, A, Q, g, E) {
    super(B, A, Q, null), this._match = new BA(g, this.id), this.captures = E, this._cachedCompiledPatterns = null;
  }
  dispose() {
    this._cachedCompiledPatterns && (this._cachedCompiledPatterns.dispose(), this._cachedCompiledPatterns = null);
  }
  get debugMatchRegExp() {
    return `${this._match.source}`;
  }
  collectPatterns(B, A) {
    A.push(this._match);
  }
  compile(B, A) {
    return this._getCachedCompiledPatterns(B).compile(B);
  }
  compileAG(B, A, Q, g) {
    return this._getCachedCompiledPatterns(B).compileAG(B, Q, g);
  }
  _getCachedCompiledPatterns(B) {
    return this._cachedCompiledPatterns || (this._cachedCompiledPatterns = new QA(), this.collectPatterns(B, this._cachedCompiledPatterns)), this._cachedCompiledPatterns;
  }
}, wB = class extends CA {
  hasMissingPatterns;
  patterns;
  _cachedCompiledPatterns;
  constructor(B, A, Q, g, E) {
    super(B, A, Q, g), this.patterns = E.patterns, this.hasMissingPatterns = E.hasMissingPatterns, this._cachedCompiledPatterns = null;
  }
  dispose() {
    this._cachedCompiledPatterns && (this._cachedCompiledPatterns.dispose(), this._cachedCompiledPatterns = null);
  }
  collectPatterns(B, A) {
    for (const Q of this.patterns)
      B.getRule(Q).collectPatterns(B, A);
  }
  compile(B, A) {
    return this._getCachedCompiledPatterns(B).compile(B);
  }
  compileAG(B, A, Q, g) {
    return this._getCachedCompiledPatterns(B).compileAG(B, Q, g);
  }
  _getCachedCompiledPatterns(B) {
    return this._cachedCompiledPatterns || (this._cachedCompiledPatterns = new QA(), this.collectPatterns(B, this._cachedCompiledPatterns)), this._cachedCompiledPatterns;
  }
}, OA = class extends CA {
  _begin;
  beginCaptures;
  _end;
  endHasBackReferences;
  endCaptures;
  applyEndPatternLast;
  hasMissingPatterns;
  patterns;
  _cachedCompiledPatterns;
  constructor(B, A, Q, g, E, C, I, w, D, o) {
    super(B, A, Q, g), this._begin = new BA(E, this.id), this.beginCaptures = C, this._end = new BA(I || "￿", -1), this.endHasBackReferences = this._end.hasBackReferences, this.endCaptures = w, this.applyEndPatternLast = D || !1, this.patterns = o.patterns, this.hasMissingPatterns = o.hasMissingPatterns, this._cachedCompiledPatterns = null;
  }
  dispose() {
    this._cachedCompiledPatterns && (this._cachedCompiledPatterns.dispose(), this._cachedCompiledPatterns = null);
  }
  get debugBeginRegExp() {
    return `${this._begin.source}`;
  }
  get debugEndRegExp() {
    return `${this._end.source}`;
  }
  getEndWithResolvedBackReferences(B, A) {
    return this._end.resolveBackReferences(B, A);
  }
  collectPatterns(B, A) {
    A.push(this._begin);
  }
  compile(B, A) {
    return this._getCachedCompiledPatterns(B, A).compile(B);
  }
  compileAG(B, A, Q, g) {
    return this._getCachedCompiledPatterns(B, A).compileAG(B, Q, g);
  }
  _getCachedCompiledPatterns(B, A) {
    if (!this._cachedCompiledPatterns) {
      this._cachedCompiledPatterns = new QA();
      for (const Q of this.patterns)
        B.getRule(Q).collectPatterns(B, this._cachedCompiledPatterns);
      this.applyEndPatternLast ? this._cachedCompiledPatterns.push(this._end.hasBackReferences ? this._end.clone() : this._end) : this._cachedCompiledPatterns.unshift(this._end.hasBackReferences ? this._end.clone() : this._end);
    }
    return this._end.hasBackReferences && (this.applyEndPatternLast ? this._cachedCompiledPatterns.setSource(this._cachedCompiledPatterns.length() - 1, A) : this._cachedCompiledPatterns.setSource(0, A)), this._cachedCompiledPatterns;
  }
}, MA = class extends CA {
  _begin;
  beginCaptures;
  whileCaptures;
  _while;
  whileHasBackReferences;
  hasMissingPatterns;
  patterns;
  _cachedCompiledPatterns;
  _cachedCompiledWhilePatterns;
  constructor(B, A, Q, g, E, C, I, w, D) {
    super(B, A, Q, g), this._begin = new BA(E, this.id), this.beginCaptures = C, this.whileCaptures = w, this._while = new BA(I, fB), this.whileHasBackReferences = this._while.hasBackReferences, this.patterns = D.patterns, this.hasMissingPatterns = D.hasMissingPatterns, this._cachedCompiledPatterns = null, this._cachedCompiledWhilePatterns = null;
  }
  dispose() {
    this._cachedCompiledPatterns && (this._cachedCompiledPatterns.dispose(), this._cachedCompiledPatterns = null), this._cachedCompiledWhilePatterns && (this._cachedCompiledWhilePatterns.dispose(), this._cachedCompiledWhilePatterns = null);
  }
  get debugBeginRegExp() {
    return `${this._begin.source}`;
  }
  get debugWhileRegExp() {
    return `${this._while.source}`;
  }
  getWhileWithResolvedBackReferences(B, A) {
    return this._while.resolveBackReferences(B, A);
  }
  collectPatterns(B, A) {
    A.push(this._begin);
  }
  compile(B, A) {
    return this._getCachedCompiledPatterns(B).compile(B);
  }
  compileAG(B, A, Q, g) {
    return this._getCachedCompiledPatterns(B).compileAG(B, Q, g);
  }
  _getCachedCompiledPatterns(B) {
    if (!this._cachedCompiledPatterns) {
      this._cachedCompiledPatterns = new QA();
      for (const A of this.patterns)
        B.getRule(A).collectPatterns(B, this._cachedCompiledPatterns);
    }
    return this._cachedCompiledPatterns;
  }
  compileWhile(B, A) {
    return this._getCachedCompiledWhilePatterns(B, A).compile(B);
  }
  compileWhileAG(B, A, Q, g) {
    return this._getCachedCompiledWhilePatterns(B, A).compileAG(B, Q, g);
  }
  _getCachedCompiledWhilePatterns(B, A) {
    return this._cachedCompiledWhilePatterns || (this._cachedCompiledWhilePatterns = new QA(), this._cachedCompiledWhilePatterns.push(this._while.hasBackReferences ? this._while.clone() : this._while)), this._while.hasBackReferences && this._cachedCompiledWhilePatterns.setSource(0, A || "￿"), this._cachedCompiledWhilePatterns;
  }
}, dB = class Y {
  static createCaptureRule(A, Q, g, E, C) {
    return A.registerRule((I) => new Ag(Q, I, g, E, C));
  }
  static getCompiledRuleId(A, Q, g) {
    return A.id || Q.registerRule((E) => {
      if (A.id = E, A.match)
        return new Bg(
          A.$vscodeTextmateLocation,
          A.id,
          A.name,
          A.match,
          Y._compileCaptures(A.captures, Q, g)
        );
      if (typeof A.begin > "u") {
        A.repository && (g = KB({}, g, A.repository));
        let C = A.patterns;
        return typeof C > "u" && A.include && (C = [{ include: A.include }]), new wB(
          A.$vscodeTextmateLocation,
          A.id,
          A.name,
          A.contentName,
          Y._compilePatterns(C, Q, g)
        );
      }
      return A.while ? new MA(
        A.$vscodeTextmateLocation,
        A.id,
        A.name,
        A.contentName,
        A.begin,
        Y._compileCaptures(A.beginCaptures || A.captures, Q, g),
        A.while,
        Y._compileCaptures(A.whileCaptures || A.captures, Q, g),
        Y._compilePatterns(A.patterns, Q, g)
      ) : new OA(
        A.$vscodeTextmateLocation,
        A.id,
        A.name,
        A.contentName,
        A.begin,
        Y._compileCaptures(A.beginCaptures || A.captures, Q, g),
        A.end,
        Y._compileCaptures(A.endCaptures || A.captures, Q, g),
        A.applyEndPatternLast,
        Y._compilePatterns(A.patterns, Q, g)
      );
    }), A.id;
  }
  static _compileCaptures(A, Q, g) {
    let E = [];
    if (A) {
      let C = 0;
      for (const I in A) {
        if (I === "$vscodeTextmateLocation")
          continue;
        const w = parseInt(I, 10);
        w > C && (C = w);
      }
      for (let I = 0; I <= C; I++)
        E[I] = null;
      for (const I in A) {
        if (I === "$vscodeTextmateLocation")
          continue;
        const w = parseInt(I, 10);
        let D = 0;
        A[I].patterns && (D = Y.getCompiledRuleId(A[I], Q, g)), E[w] = Y.createCaptureRule(Q, A[I].$vscodeTextmateLocation, A[I].name, A[I].contentName, D);
      }
    }
    return E;
  }
  static _compilePatterns(A, Q, g) {
    let E = [];
    if (A)
      for (let C = 0, I = A.length; C < I; C++) {
        const w = A[C];
        let D = -1;
        if (w.include) {
          const o = pB(w.include);
          switch (o.kind) {
            case 0:
            case 1:
              D = Y.getCompiledRuleId(g[w.include], Q, g);
              break;
            case 2:
              let s = g[o.ruleName];
              s && (D = Y.getCompiledRuleId(s, Q, g));
              break;
            case 3:
            case 4:
              const e = o.scopeName, n = o.kind === 4 ? o.ruleName : null, i = Q.getExternalGrammar(e, g);
              if (i)
                if (n) {
                  let t = i.repository[n];
                  t && (D = Y.getCompiledRuleId(t, Q, i.repository));
                } else
                  D = Y.getCompiledRuleId(i.repository.$self, Q, i.repository);
              break;
          }
        } else
          D = Y.getCompiledRuleId(w, Q, g);
        if (D !== -1) {
          const o = Q.getRule(D);
          let s = !1;
          if ((o instanceof wB || o instanceof OA || o instanceof MA) && o.hasMissingPatterns && o.patterns.length === 0 && (s = !0), s)
            continue;
          E.push(D);
        }
      }
    return {
      patterns: E,
      hasMissingPatterns: (A ? A.length : 0) !== E.length
    };
  }
}, BA = class OB {
  source;
  ruleId;
  hasAnchor;
  hasBackReferences;
  _anchorCache;
  constructor(A, Q) {
    if (A && typeof A == "string") {
      const g = A.length;
      let E = 0, C = [], I = !1;
      for (let w = 0; w < g; w++)
        if (A.charAt(w) === "\\" && w + 1 < g) {
          const o = A.charAt(w + 1);
          o === "z" ? (C.push(A.substring(E, w)), C.push("$(?!\\n)(?<!\\n)"), E = w + 2) : (o === "A" || o === "G") && (I = !0), w++;
        }
      this.hasAnchor = I, E === 0 ? this.source = A : (C.push(A.substring(E, g)), this.source = C.join(""));
    } else
      this.hasAnchor = !1, this.source = A;
    this.hasAnchor ? this._anchorCache = this._buildAnchorCache() : this._anchorCache = null, this.ruleId = Q, typeof this.source == "string" ? this.hasBackReferences = _Q.test(this.source) : this.hasBackReferences = !1;
  }
  clone() {
    return new OB(this.source, this.ruleId);
  }
  setSource(A) {
    this.source !== A && (this.source = A, this.hasAnchor && (this._anchorCache = this._buildAnchorCache()));
  }
  resolveBackReferences(A, Q) {
    if (typeof this.source != "string")
      throw new Error("This method should only be called if the source is a string");
    let g = Q.map((E) => A.substring(E.start, E.end));
    return DB.lastIndex = 0, this.source.replace(DB, (E, C) => PB(g[parseInt(C, 10)] || ""));
  }
  _buildAnchorCache() {
    if (typeof this.source != "string")
      throw new Error("This method should only be called if the source is a string");
    let A = [], Q = [], g = [], E = [], C, I, w, D;
    for (C = 0, I = this.source.length; C < I; C++)
      w = this.source.charAt(C), A[C] = w, Q[C] = w, g[C] = w, E[C] = w, w === "\\" && C + 1 < I && (D = this.source.charAt(C + 1), D === "A" ? (A[C + 1] = "￿", Q[C + 1] = "￿", g[C + 1] = "A", E[C + 1] = "A") : D === "G" ? (A[C + 1] = "￿", Q[C + 1] = "G", g[C + 1] = "￿", E[C + 1] = "G") : (A[C + 1] = D, Q[C + 1] = D, g[C + 1] = D, E[C + 1] = D), C++);
    return {
      A0_G0: A.join(""),
      A0_G1: Q.join(""),
      A1_G0: g.join(""),
      A1_G1: E.join("")
    };
  }
  resolveAnchors(A, Q) {
    return !this.hasAnchor || !this._anchorCache || typeof this.source != "string" ? this.source : A ? Q ? this._anchorCache.A1_G1 : this._anchorCache.A1_G0 : Q ? this._anchorCache.A0_G1 : this._anchorCache.A0_G0;
  }
}, QA = class {
  _items;
  _hasAnchors;
  _cached;
  _anchorCache;
  constructor() {
    this._items = [], this._hasAnchors = !1, this._cached = null, this._anchorCache = {
      A0_G0: null,
      A0_G1: null,
      A1_G0: null,
      A1_G1: null
    };
  }
  dispose() {
    this._disposeCaches();
  }
  _disposeCaches() {
    this._cached && (this._cached.dispose(), this._cached = null), this._anchorCache.A0_G0 && (this._anchorCache.A0_G0.dispose(), this._anchorCache.A0_G0 = null), this._anchorCache.A0_G1 && (this._anchorCache.A0_G1.dispose(), this._anchorCache.A0_G1 = null), this._anchorCache.A1_G0 && (this._anchorCache.A1_G0.dispose(), this._anchorCache.A1_G0 = null), this._anchorCache.A1_G1 && (this._anchorCache.A1_G1.dispose(), this._anchorCache.A1_G1 = null);
  }
  push(B) {
    this._items.push(B), this._hasAnchors = this._hasAnchors || B.hasAnchor;
  }
  unshift(B) {
    this._items.unshift(B), this._hasAnchors = this._hasAnchors || B.hasAnchor;
  }
  length() {
    return this._items.length;
  }
  setSource(B, A) {
    this._items[B].source !== A && (this._disposeCaches(), this._items[B].setSource(A));
  }
  compile(B) {
    if (!this._cached) {
      let A = this._items.map((Q) => Q.source);
      this._cached = new oB(B, A, this._items.map((Q) => Q.ruleId));
    }
    return this._cached;
  }
  compileAG(B, A, Q) {
    return this._hasAnchors ? A ? Q ? (this._anchorCache.A1_G1 || (this._anchorCache.A1_G1 = this._resolveAnchors(B, A, Q)), this._anchorCache.A1_G1) : (this._anchorCache.A1_G0 || (this._anchorCache.A1_G0 = this._resolveAnchors(B, A, Q)), this._anchorCache.A1_G0) : Q ? (this._anchorCache.A0_G1 || (this._anchorCache.A0_G1 = this._resolveAnchors(B, A, Q)), this._anchorCache.A0_G1) : (this._anchorCache.A0_G0 || (this._anchorCache.A0_G0 = this._resolveAnchors(B, A, Q)), this._anchorCache.A0_G0) : this.compile(B);
  }
  _resolveAnchors(B, A, Q) {
    let g = this._items.map((E) => E.resolveAnchors(A, Q));
    return new oB(B, g, this._items.map((E) => E.ruleId));
  }
}, oB = class {
  constructor(B, A, Q) {
    this.regExps = A, this.rules = Q, this.scanner = B.createOnigScanner(A);
  }
  scanner;
  dispose() {
    typeof this.scanner.dispose == "function" && this.scanner.dispose();
  }
  toString() {
    const B = [];
    for (let A = 0, Q = this.rules.length; A < Q; A++)
      B.push("   - " + this.rules[A] + ": " + this.regExps[A]);
    return B.join(`
`);
  }
  findNextMatchSync(B, A, Q) {
    const g = this.scanner.findNextMatchSync(B, A, Q);
    return g ? {
      ruleId: this.rules[g.index],
      captureIndices: g.captureIndices
    } : null;
  }
}, PA = class {
  constructor(B, A) {
    this.languageId = B, this.tokenType = A;
  }
}, Qg = class xA {
  _defaultAttributes;
  _embeddedLanguagesMatcher;
  constructor(A, Q) {
    this._defaultAttributes = new PA(
      A,
      8
      /* NotSet */
    ), this._embeddedLanguagesMatcher = new gg(Object.entries(Q || {}));
  }
  getDefaultAttributes() {
    return this._defaultAttributes;
  }
  getBasicScopeAttributes(A) {
    return A === null ? xA._NULL_SCOPE_METADATA : this._getBasicScopeAttributes.get(A);
  }
  static _NULL_SCOPE_METADATA = new PA(0, 0);
  _getBasicScopeAttributes = new yB((A) => {
    const Q = this._scopeToLanguage(A), g = this._toStandardTokenType(A);
    return new PA(Q, g);
  });
  /**
   * Given a produced TM scope, return the language that token describes or null if unknown.
   * e.g. source.html => html, source.css.embedded.html => css, punctuation.definition.tag.html => null
   */
  _scopeToLanguage(A) {
    return this._embeddedLanguagesMatcher.match(A) || 0;
  }
  _toStandardTokenType(A) {
    const Q = A.match(xA.STANDARD_TOKEN_TYPE_REGEXP);
    if (!Q)
      return 8;
    switch (Q[1]) {
      case "comment":
        return 1;
      case "string":
        return 2;
      case "regex":
        return 3;
      case "meta.embedded":
        return 0;
    }
    throw new Error("Unexpected match for standard token type!");
  }
  static STANDARD_TOKEN_TYPE_REGEXP = /\b(comment|string|regex|meta\.embedded)\b/;
}, gg = class {
  values;
  scopesRegExp;
  constructor(B) {
    if (B.length === 0)
      this.values = null, this.scopesRegExp = null;
    else {
      this.values = new Map(B);
      const A = B.map(
        ([Q, g]) => PB(Q)
      );
      A.sort(), A.reverse(), this.scopesRegExp = new RegExp(
        `^((${A.join(")|(")}))($|\\.)`,
        ""
      );
    }
  }
  match(B) {
    if (!this.scopesRegExp)
      return;
    const A = B.match(this.scopesRegExp);
    if (A)
      return this.values.get(A[1]);
  }
};
typeof process < "u" && process.env.VSCODE_TEXTMATE_DEBUG;
var sB = class {
  constructor(B, A) {
    this.stack = B, this.stoppedEarly = A;
  }
};
function xB(B, A, Q, g, E, C, I, w) {
  const D = A.content.length;
  let o = !1, s = -1;
  if (I) {
    const i = Eg(
      B,
      A,
      Q,
      g,
      E,
      C
    );
    E = i.stack, g = i.linePos, Q = i.isFirstLine, s = i.anchorPosition;
  }
  const e = Date.now();
  for (; !o; ) {
    if (w !== 0 && Date.now() - e > w)
      return new sB(E, !0);
    n();
  }
  return new sB(E, !1);
  function n() {
    const i = Cg(
      B,
      A,
      Q,
      g,
      E,
      s
    );
    if (!i) {
      C.produce(E, D), o = !0;
      return;
    }
    const t = i.captureIndices, N = i.matchedRuleId, M = t && t.length > 0 ? t[0].end > g : !1;
    if (N === $Q) {
      const h = E.getRule(B);
      C.produce(E, t[0].start), E = E.withContentNameScopesList(E.nameScopesList), _(
        B,
        A,
        Q,
        E,
        C,
        h.endCaptures,
        t
      ), C.produce(E, t[0].end);
      const a = E;
      if (E = E.parent, s = a.getAnchorPos(), !M && a.getEnterPos() === g) {
        E = a, C.produce(E, D), o = !0;
        return;
      }
    } else {
      const h = B.getRule(N);
      C.produce(E, t[0].start);
      const a = E, F = h.getName(A.content, t), R = E.contentNameScopesList.pushAttributed(
        F,
        B
      );
      if (E = E.push(
        N,
        g,
        s,
        t[0].end === D,
        null,
        R,
        R
      ), h instanceof OA) {
        const L = h;
        _(
          B,
          A,
          Q,
          E,
          C,
          L.beginCaptures,
          t
        ), C.produce(E, t[0].end), s = t[0].end;
        const u = L.getContentName(
          A.content,
          t
        ), O = R.pushAttributed(
          u,
          B
        );
        if (E = E.withContentNameScopesList(O), L.endHasBackReferences && (E = E.withEndRule(
          L.getEndWithResolvedBackReferences(
            A.content,
            t
          )
        )), !M && a.hasSameRuleAs(E)) {
          E = E.pop(), C.produce(E, D), o = !0;
          return;
        }
      } else if (h instanceof MA) {
        const L = h;
        _(
          B,
          A,
          Q,
          E,
          C,
          L.beginCaptures,
          t
        ), C.produce(E, t[0].end), s = t[0].end;
        const u = L.getContentName(
          A.content,
          t
        ), O = R.pushAttributed(
          u,
          B
        );
        if (E = E.withContentNameScopesList(O), L.whileHasBackReferences && (E = E.withEndRule(
          L.getWhileWithResolvedBackReferences(
            A.content,
            t
          )
        )), !M && a.hasSameRuleAs(E)) {
          E = E.pop(), C.produce(E, D), o = !0;
          return;
        }
      } else if (_(
        B,
        A,
        Q,
        E,
        C,
        h.captures,
        t
      ), C.produce(E, t[0].end), E = E.pop(), !M) {
        E = E.safePop(), C.produce(E, D), o = !0;
        return;
      }
    }
    t[0].end > g && (g = t[0].end, Q = !1);
  }
}
function Eg(B, A, Q, g, E, C) {
  let I = E.beginRuleCapturedEOL ? 0 : -1;
  const w = [];
  for (let D = E; D; D = D.pop()) {
    const o = D.getRule(B);
    o instanceof MA && w.push({
      rule: o,
      stack: D
    });
  }
  for (let D = w.pop(); D; D = w.pop()) {
    const { ruleScanner: o, findOptions: s } = wg(D.rule, B, D.stack.endRule, Q, g === I), e = o.findNextMatchSync(A, g, s);
    if (e) {
      if (e.ruleId !== fB) {
        E = D.stack.pop();
        break;
      }
      e.captureIndices && e.captureIndices.length && (C.produce(D.stack, e.captureIndices[0].start), _(B, A, Q, D.stack, C, D.rule.whileCaptures, e.captureIndices), C.produce(D.stack, e.captureIndices[0].end), I = e.captureIndices[0].end, e.captureIndices[0].end > g && (g = e.captureIndices[0].end, Q = !1));
    } else {
      E = D.stack.pop();
      break;
    }
  }
  return { stack: E, linePos: g, anchorPosition: I, isFirstLine: Q };
}
function Cg(B, A, Q, g, E, C) {
  const I = Ig(B, A, Q, g, E, C), w = B.getInjections();
  if (w.length === 0)
    return I;
  const D = Dg(w, B, A, Q, g, E, C);
  if (!D)
    return I;
  if (!I)
    return D;
  const o = I.captureIndices[0].start, s = D.captureIndices[0].start;
  return s < o || D.priorityMatch && s === o ? D : I;
}
function Ig(B, A, Q, g, E, C) {
  const I = E.getRule(B), { ruleScanner: w, findOptions: D } = qB(I, B, E.endRule, Q, g === C), o = w.findNextMatchSync(A, g, D);
  return o ? {
    captureIndices: o.captureIndices,
    matchedRuleId: o.ruleId
  } : null;
}
function Dg(B, A, Q, g, E, C, I) {
  let w = Number.MAX_VALUE, D = null, o, s = 0;
  const e = C.contentNameScopesList.getScopeNames();
  for (let n = 0, i = B.length; n < i; n++) {
    const t = B[n];
    if (!t.matcher(e))
      continue;
    const N = A.getRule(t.ruleId), { ruleScanner: M, findOptions: h } = qB(N, A, null, g, E === I), a = M.findNextMatchSync(Q, E, h);
    if (!a)
      continue;
    const F = a.captureIndices[0].start;
    if (!(F >= w) && (w = F, D = a.captureIndices, o = a.ruleId, s = t.priority, w === E))
      break;
  }
  return D ? {
    priorityMatch: s === -1,
    captureIndices: D,
    matchedRuleId: o
  } : null;
}
function qB(B, A, Q, g, E) {
  return {
    ruleScanner: B.compileAG(A, Q, g, E),
    findOptions: 0
    /* None */
  };
}
function wg(B, A, Q, g, E) {
  return {
    ruleScanner: B.compileWhileAG(A, Q, g, E),
    findOptions: 0
    /* None */
  };
}
function _(B, A, Q, g, E, C, I) {
  if (C.length === 0)
    return;
  const w = A.content, D = Math.min(C.length, I.length), o = [], s = I[0].end;
  for (let e = 0; e < D; e++) {
    const n = C[e];
    if (n === null)
      continue;
    const i = I[e];
    if (i.length === 0)
      continue;
    if (i.start > s)
      break;
    for (; o.length > 0 && o[o.length - 1].endPos <= i.start; )
      E.produceFromScopes(o[o.length - 1].scopes, o[o.length - 1].endPos), o.pop();
    if (o.length > 0 ? E.produceFromScopes(o[o.length - 1].scopes, i.start) : E.produce(g, i.start), n.retokenizeCapturedWithRuleId) {
      const N = n.getName(w, I), M = g.contentNameScopesList.pushAttributed(N, B), h = n.getContentName(w, I), a = M.pushAttributed(h, B), F = g.push(n.retokenizeCapturedWithRuleId, i.start, -1, !1, null, M, a), R = B.createOnigString(w.substring(0, i.end));
      xB(
        B,
        R,
        Q && i.start === 0,
        i.start,
        F,
        E,
        !1,
        /* no time limit */
        0
      ), SB(R);
      continue;
    }
    const t = n.getName(w, I);
    if (t !== null) {
      const M = (o.length > 0 ? o[o.length - 1].scopes : g.contentNameScopesList).pushAttributed(t, B);
      o.push(new og(M, i.end));
    }
  }
  for (; o.length > 0; )
    E.produceFromScopes(o[o.length - 1].scopes, o[o.length - 1].endPos), o.pop();
}
var og = class {
  scopes;
  endPos;
  constructor(B, A) {
    this.scopes = B, this.endPos = A;
  }
};
function sg(B, A, Q, g, E, C, I, w) {
  return new ig(
    B,
    A,
    Q,
    g,
    E,
    C,
    I,
    w
  );
}
function eB(B, A, Q, g, E) {
  const C = cA(A, aA), I = dB.getCompiledRuleId(Q, g, E.repository);
  for (const w of C)
    B.push({
      debugSelector: A,
      matcher: w.matcher,
      ruleId: I,
      grammar: E,
      priority: w.priority
    });
}
function aA(B, A) {
  if (A.length < B.length)
    return !1;
  let Q = 0;
  return B.every((g) => {
    for (let E = Q; E < A.length; E++)
      if (eg(A[E], g))
        return Q = E + 1, !0;
    return !1;
  });
}
function eg(B, A) {
  if (!B)
    return !1;
  if (B === A)
    return !0;
  const Q = A.length;
  return B.length > Q && B.substr(0, Q) === A && B[Q] === ".";
}
var ig = class {
  constructor(B, A, Q, g, E, C, I, w) {
    if (this._rootScopeName = B, this.balancedBracketSelectors = C, this._onigLib = w, this._basicScopeAttributesProvider = new Qg(
      Q,
      g
    ), this._rootId = -1, this._lastRuleId = 0, this._ruleId2desc = [null], this._includedGrammars = {}, this._grammarRepository = I, this._grammar = iB(A, null), this._injections = null, this._tokenTypeMatchers = [], E)
      for (const D of Object.keys(E)) {
        const o = cA(D, aA);
        for (const s of o)
          this._tokenTypeMatchers.push({
            matcher: s.matcher,
            type: E[D]
          });
      }
  }
  _rootId;
  _lastRuleId;
  _ruleId2desc;
  _includedGrammars;
  _grammarRepository;
  _grammar;
  _injections;
  _basicScopeAttributesProvider;
  _tokenTypeMatchers;
  get themeProvider() {
    return this._grammarRepository;
  }
  dispose() {
    for (const B of this._ruleId2desc)
      B && B.dispose();
  }
  createOnigScanner(B) {
    return this._onigLib.createOnigScanner(B);
  }
  createOnigString(B) {
    return this._onigLib.createOnigString(B);
  }
  getMetadataForScope(B) {
    return this._basicScopeAttributesProvider.getBasicScopeAttributes(B);
  }
  _collectInjections() {
    const B = {
      lookup: (E) => E === this._rootScopeName ? this._grammar : this.getExternalGrammar(E),
      injections: (E) => this._grammarRepository.injections(E)
    }, A = [], Q = this._rootScopeName, g = B.lookup(Q);
    if (g) {
      const E = g.injections;
      if (E)
        for (let I in E)
          eB(
            A,
            I,
            E[I],
            this,
            g
          );
      const C = this._grammarRepository.injections(Q);
      C && C.forEach((I) => {
        const w = this.getExternalGrammar(I);
        if (w) {
          const D = w.injectionSelector;
          D && eB(
            A,
            D,
            w,
            this,
            w
          );
        }
      });
    }
    return A.sort((E, C) => E.priority - C.priority), A;
  }
  getInjections() {
    return this._injections === null && (this._injections = this._collectInjections()), this._injections;
  }
  registerRule(B) {
    const A = ++this._lastRuleId, Q = B(A);
    return this._ruleId2desc[A] = Q, Q;
  }
  getRule(B) {
    return this._ruleId2desc[B];
  }
  getExternalGrammar(B, A) {
    if (this._includedGrammars[B])
      return this._includedGrammars[B];
    if (this._grammarRepository) {
      const Q = this._grammarRepository.lookup(B);
      if (Q)
        return this._includedGrammars[B] = iB(
          Q,
          A && A.$base
        ), this._includedGrammars[B];
    }
  }
  tokenizeLine(B, A, Q = 0) {
    const g = this._tokenize(B, A, !1, Q);
    return {
      tokens: g.lineTokens.getResult(g.ruleStack, g.lineLength),
      ruleStack: g.ruleStack,
      stoppedEarly: g.stoppedEarly
    };
  }
  tokenizeLine2(B, A, Q = 0) {
    const g = this._tokenize(B, A, !0, Q);
    return {
      tokens: g.lineTokens.getBinaryResult(g.ruleStack, g.lineLength),
      ruleStack: g.ruleStack,
      stoppedEarly: g.stoppedEarly
    };
  }
  _tokenize(B, A, Q, g) {
    this._rootId === -1 && (this._rootId = dB.getCompiledRuleId(
      this._grammar.repository.$self,
      this,
      this._grammar.repository
    ), this.getInjections());
    let E;
    if (!A || A === qA.NULL) {
      E = !0;
      const o = this._basicScopeAttributesProvider.getDefaultAttributes(), s = this.themeProvider.getDefaults(), e = v.set(
        0,
        o.languageId,
        o.tokenType,
        null,
        s.fontStyle,
        s.foregroundId,
        s.backgroundId
      ), n = this.getRule(this._rootId).getName(
        null,
        null
      );
      let i;
      n ? i = $.createRootAndLookUpScopeName(
        n,
        e,
        this
      ) : i = $.createRoot(
        "unknown",
        e
      ), A = new qA(
        null,
        this._rootId,
        -1,
        -1,
        !1,
        null,
        i,
        i
      );
    } else
      E = !1, A.reset();
    B = B + `
`;
    const C = this.createOnigString(B), I = C.content.length, w = new cg(
      Q,
      B,
      this._tokenTypeMatchers,
      this.balancedBracketSelectors
    ), D = xB(
      this,
      C,
      E,
      0,
      A,
      w,
      !0,
      g
    );
    return SB(C), {
      lineLength: I,
      lineTokens: w,
      ruleStack: D.stack,
      stoppedEarly: D.stoppedEarly
    };
  }
};
function iB(B, A) {
  return B = PQ(B), B.repository = B.repository || {}, B.repository.$self = {
    $vscodeTextmateLocation: B.$vscodeTextmateLocation,
    patterns: B.patterns,
    name: B.scopeName
  }, B.repository.$base = A || B.repository.$self, B;
}
var $ = class p {
  /**
   * Invariant:
   * ```
   * if (parent && !scopePath.extends(parent.scopePath)) {
   * 	throw new Error();
   * }
   * ```
   */
  constructor(A, Q, g) {
    this.parent = A, this.scopePath = Q, this.tokenAttributes = g;
  }
  static fromExtension(A, Q) {
    let g = A, E = A?.scopePath ?? null;
    for (const C of Q)
      E = lA.push(E, C.scopeNames), g = new p(g, E, C.encodedTokenAttributes);
    return g;
  }
  static createRoot(A, Q) {
    return new p(null, new lA(null, A), Q);
  }
  static createRootAndLookUpScopeName(A, Q, g) {
    const E = g.getMetadataForScope(A), C = new lA(null, A), I = g.themeProvider.themeMatch(C), w = p.mergeAttributes(
      Q,
      E,
      I
    );
    return new p(null, C, w);
  }
  get scopeName() {
    return this.scopePath.scopeName;
  }
  toString() {
    return this.getScopeNames().join(" ");
  }
  equals(A) {
    return p.equals(this, A);
  }
  static equals(A, Q) {
    do {
      if (A === Q || !A && !Q)
        return !0;
      if (!A || !Q || A.scopeName !== Q.scopeName || A.tokenAttributes !== Q.tokenAttributes)
        return !1;
      A = A.parent, Q = Q.parent;
    } while (!0);
  }
  static mergeAttributes(A, Q, g) {
    let E = -1, C = 0, I = 0;
    return g !== null && (E = g.fontStyle, C = g.foregroundId, I = g.backgroundId), v.set(
      A,
      Q.languageId,
      Q.tokenType,
      null,
      E,
      C,
      I
    );
  }
  pushAttributed(A, Q) {
    if (A === null)
      return this;
    if (A.indexOf(" ") === -1)
      return p._pushAttributed(this, A, Q);
    const g = A.split(/ /g);
    let E = this;
    for (const C of g)
      E = p._pushAttributed(E, C, Q);
    return E;
  }
  static _pushAttributed(A, Q, g) {
    const E = g.getMetadataForScope(Q), C = A.scopePath.push(Q), I = g.themeProvider.themeMatch(C), w = p.mergeAttributes(
      A.tokenAttributes,
      E,
      I
    );
    return new p(A, C, w);
  }
  getScopeNames() {
    return this.scopePath.getSegments();
  }
  getExtensionIfDefined(A) {
    const Q = [];
    let g = this;
    for (; g && g !== A; )
      Q.push({
        encodedTokenAttributes: g.tokenAttributes,
        scopeNames: g.scopePath.getExtensionIfDefined(g.parent?.scopePath ?? null)
      }), g = g.parent;
    return g === A ? Q.reverse() : void 0;
  }
}, qA = class x {
  /**
   * Invariant:
   * ```
   * if (contentNameScopesList !== nameScopesList && contentNameScopesList?.parent !== nameScopesList) {
   * 	throw new Error();
   * }
   * if (this.parent && !nameScopesList.extends(this.parent.contentNameScopesList)) {
   * 	throw new Error();
   * }
   * ```
   */
  constructor(A, Q, g, E, C, I, w, D) {
    this.parent = A, this.ruleId = Q, this.beginRuleCapturedEOL = C, this.endRule = I, this.nameScopesList = w, this.contentNameScopesList = D, this.depth = this.parent ? this.parent.depth + 1 : 1, this._enterPos = g, this._anchorPos = E;
  }
  _stackElementBrand = void 0;
  // TODO remove me
  static NULL = new x(
    null,
    0,
    0,
    0,
    !1,
    null,
    null,
    null
  );
  /**
   * The position on the current line where this state was pushed.
   * This is relevant only while tokenizing a line, to detect endless loops.
   * Its value is meaningless across lines.
   */
  _enterPos;
  /**
   * The captured anchor position when this stack element was pushed.
   * This is relevant only while tokenizing a line, to restore the anchor position when popping.
   * Its value is meaningless across lines.
   */
  _anchorPos;
  /**
   * The depth of the stack.
   */
  depth;
  equals(A) {
    return A === null ? !1 : x._equals(this, A);
  }
  static _equals(A, Q) {
    return A === Q ? !0 : this._structuralEquals(A, Q) ? $.equals(A.contentNameScopesList, Q.contentNameScopesList) : !1;
  }
  /**
   * A structural equals check. Does not take into account `scopes`.
   */
  static _structuralEquals(A, Q) {
    do {
      if (A === Q || !A && !Q)
        return !0;
      if (!A || !Q || A.depth !== Q.depth || A.ruleId !== Q.ruleId || A.endRule !== Q.endRule)
        return !1;
      A = A.parent, Q = Q.parent;
    } while (!0);
  }
  clone() {
    return this;
  }
  static _reset(A) {
    for (; A; )
      A._enterPos = -1, A._anchorPos = -1, A = A.parent;
  }
  reset() {
    x._reset(this);
  }
  pop() {
    return this.parent;
  }
  safePop() {
    return this.parent ? this.parent : this;
  }
  push(A, Q, g, E, C, I, w) {
    return new x(
      this,
      A,
      Q,
      g,
      E,
      C,
      I,
      w
    );
  }
  getEnterPos() {
    return this._enterPos;
  }
  getAnchorPos() {
    return this._anchorPos;
  }
  getRule(A) {
    return A.getRule(this.ruleId);
  }
  toString() {
    const A = [];
    return this._writeString(A, 0), "[" + A.join(",") + "]";
  }
  _writeString(A, Q) {
    return this.parent && (Q = this.parent._writeString(A, Q)), A[Q++] = `(${this.ruleId}, ${this.nameScopesList?.toString()}, ${this.contentNameScopesList?.toString()})`, Q;
  }
  withContentNameScopesList(A) {
    return this.contentNameScopesList === A ? this : this.parent.push(
      this.ruleId,
      this._enterPos,
      this._anchorPos,
      this.beginRuleCapturedEOL,
      this.endRule,
      this.nameScopesList,
      A
    );
  }
  withEndRule(A) {
    return this.endRule === A ? this : new x(
      this.parent,
      this.ruleId,
      this._enterPos,
      this._anchorPos,
      this.beginRuleCapturedEOL,
      A,
      this.nameScopesList,
      this.contentNameScopesList
    );
  }
  // Used to warn of endless loops
  hasSameRuleAs(A) {
    let Q = this;
    for (; Q && Q._enterPos === A._enterPos; ) {
      if (Q.ruleId === A.ruleId)
        return !0;
      Q = Q.parent;
    }
    return !1;
  }
  toStateStackFrame() {
    return {
      ruleId: this.ruleId,
      beginRuleCapturedEOL: this.beginRuleCapturedEOL,
      endRule: this.endRule,
      nameScopesList: this.nameScopesList?.getExtensionIfDefined(this.parent?.nameScopesList ?? null) ?? [],
      contentNameScopesList: this.contentNameScopesList?.getExtensionIfDefined(this.nameScopesList) ?? []
    };
  }
  static pushFrame(A, Q) {
    const g = $.fromExtension(A?.nameScopesList ?? null, Q.nameScopesList);
    return new x(
      A,
      Q.ruleId,
      Q.enterPos ?? -1,
      Q.anchorPos ?? -1,
      Q.beginRuleCapturedEOL,
      Q.endRule,
      g,
      $.fromExtension(g, Q.contentNameScopesList)
    );
  }
}, tg = class {
  balancedBracketScopes;
  unbalancedBracketScopes;
  allowAny = !1;
  constructor(B, A) {
    this.balancedBracketScopes = B.flatMap(
      (Q) => Q === "*" ? (this.allowAny = !0, []) : cA(Q, aA).map((g) => g.matcher)
    ), this.unbalancedBracketScopes = A.flatMap(
      (Q) => cA(Q, aA).map((g) => g.matcher)
    );
  }
  get matchesAlways() {
    return this.allowAny && this.unbalancedBracketScopes.length === 0;
  }
  get matchesNever() {
    return this.balancedBracketScopes.length === 0 && !this.allowAny;
  }
  match(B) {
    for (const A of this.unbalancedBracketScopes)
      if (A(B))
        return !1;
    for (const A of this.balancedBracketScopes)
      if (A(B))
        return !0;
    return this.allowAny;
  }
}, cg = class {
  constructor(B, A, Q, g) {
    this.balancedBracketSelectors = g, this._emitBinaryTokens = B, this._tokenTypeOverrides = Q, this._lineText = null, this._tokens = [], this._binaryTokens = [], this._lastTokenEndIndex = 0;
  }
  _emitBinaryTokens;
  /**
   * defined only if `false`.
   */
  _lineText;
  /**
   * used only if `_emitBinaryTokens` is false.
   */
  _tokens;
  /**
   * used only if `_emitBinaryTokens` is true.
   */
  _binaryTokens;
  _lastTokenEndIndex;
  _tokenTypeOverrides;
  produce(B, A) {
    this.produceFromScopes(B.contentNameScopesList, A);
  }
  produceFromScopes(B, A) {
    if (this._lastTokenEndIndex >= A)
      return;
    if (this._emitBinaryTokens) {
      let g = B?.tokenAttributes ?? 0, E = !1;
      if (this.balancedBracketSelectors?.matchesAlways && (E = !0), this._tokenTypeOverrides.length > 0 || this.balancedBracketSelectors && !this.balancedBracketSelectors.matchesAlways && !this.balancedBracketSelectors.matchesNever) {
        const C = B?.getScopeNames() ?? [];
        for (const I of this._tokenTypeOverrides)
          I.matcher(C) && (g = v.set(
            g,
            0,
            I.type,
            null,
            -1,
            0,
            0
          ));
        this.balancedBracketSelectors && (E = this.balancedBracketSelectors.match(C));
      }
      if (E && (g = v.set(
        g,
        0,
        8,
        E,
        -1,
        0,
        0
      )), this._binaryTokens.length > 0 && this._binaryTokens[this._binaryTokens.length - 1] === g) {
        this._lastTokenEndIndex = A;
        return;
      }
      this._binaryTokens.push(this._lastTokenEndIndex), this._binaryTokens.push(g), this._lastTokenEndIndex = A;
      return;
    }
    const Q = B?.getScopeNames() ?? [];
    this._tokens.push({
      startIndex: this._lastTokenEndIndex,
      endIndex: A,
      // value: lineText.substring(lastTokenEndIndex, endIndex),
      scopes: Q
    }), this._lastTokenEndIndex = A;
  }
  getResult(B, A) {
    return this._tokens.length > 0 && this._tokens[this._tokens.length - 1].startIndex === A - 1 && this._tokens.pop(), this._tokens.length === 0 && (this._lastTokenEndIndex = -1, this.produce(B, A), this._tokens[this._tokens.length - 1].startIndex = 0), this._tokens;
  }
  getBinaryResult(B, A) {
    this._binaryTokens.length > 0 && this._binaryTokens[this._binaryTokens.length - 2] === A - 1 && (this._binaryTokens.pop(), this._binaryTokens.pop()), this._binaryTokens.length === 0 && (this._lastTokenEndIndex = -1, this.produce(B, A), this._binaryTokens[this._binaryTokens.length - 2] = 0);
    const Q = new Uint32Array(this._binaryTokens.length);
    for (let g = 0, E = this._binaryTokens.length; g < E; g++)
      Q[g] = this._binaryTokens[g];
    return Q;
  }
}, ng = class {
  constructor(B, A) {
    this._onigLib = A, this._theme = B;
  }
  _grammars = /* @__PURE__ */ new Map();
  _rawGrammars = /* @__PURE__ */ new Map();
  _injectionGrammars = /* @__PURE__ */ new Map();
  _theme;
  dispose() {
    for (const B of this._grammars.values())
      B.dispose();
  }
  setTheme(B) {
    this._theme = B;
  }
  getColorMap() {
    return this._theme.getColorMap();
  }
  /**
   * Add `grammar` to registry and return a list of referenced scope names
   */
  addGrammar(B, A) {
    this._rawGrammars.set(B.scopeName, B), A && this._injectionGrammars.set(B.scopeName, A);
  }
  /**
   * Lookup a raw grammar.
   */
  lookup(B) {
    return this._rawGrammars.get(B);
  }
  /**
   * Returns the injections for the given grammar
   */
  injections(B) {
    return this._injectionGrammars.get(B);
  }
  /**
   * Get the default theme settings
   */
  getDefaults() {
    return this._theme.getDefaults();
  }
  /**
   * Match a scope in the theme.
   */
  themeMatch(B) {
    return this._theme.match(B);
  }
  /**
   * Lookup a grammar.
   */
  grammarForScopeName(B, A, Q, g, E) {
    if (!this._grammars.has(B)) {
      let C = this._rawGrammars.get(B);
      if (!C)
        return null;
      this._grammars.set(B, sg(
        B,
        C,
        A,
        Q,
        g,
        E,
        this,
        this._onigLib
      ));
    }
    return this._grammars.get(B);
  }
}, Mg = class {
  _options;
  _syncRegistry;
  _ensureGrammarCache;
  constructor(A) {
    this._options = A, this._syncRegistry = new ng(
      tA.createFromRawTheme(A.theme, A.colorMap),
      A.onigLib
    ), this._ensureGrammarCache = /* @__PURE__ */ new Map();
  }
  dispose() {
    this._syncRegistry.dispose();
  }
  /**
   * Change the theme. Once called, no previous `ruleStack` should be used anymore.
   */
  setTheme(A, Q) {
    this._syncRegistry.setTheme(tA.createFromRawTheme(A, Q));
  }
  /**
   * Returns a lookup array for color ids.
   */
  getColorMap() {
    return this._syncRegistry.getColorMap();
  }
  /**
   * Load the grammar for `scopeName` and all referenced included grammars asynchronously.
   * Please do not use language id 0.
   */
  loadGrammarWithEmbeddedLanguages(A, Q, g) {
    return this.loadGrammarWithConfiguration(A, Q, { embeddedLanguages: g });
  }
  /**
   * Load the grammar for `scopeName` and all referenced included grammars asynchronously.
   * Please do not use language id 0.
   */
  loadGrammarWithConfiguration(A, Q, g) {
    return this._loadGrammar(
      A,
      Q,
      g.embeddedLanguages,
      g.tokenTypes,
      new tg(
        g.balancedBracketSelectors || [],
        g.unbalancedBracketSelectors || []
      )
    );
  }
  /**
   * Load the grammar for `scopeName` and all referenced included grammars asynchronously.
   */
  loadGrammar(A) {
    return this._loadGrammar(A, 0, null, null, null);
  }
  _loadGrammar(A, Q, g, E, C) {
    const I = new WQ(this._syncRegistry, A);
    for (; I.Q.length > 0; )
      I.Q.map((w) => this._loadSingleGrammar(w.scopeName)), I.processQueue();
    return this._grammarForScopeName(
      A,
      Q,
      g,
      E,
      C
    );
  }
  _loadSingleGrammar(A) {
    this._ensureGrammarCache.has(A) || (this._doLoadSingleGrammar(A), this._ensureGrammarCache.set(A, !0));
  }
  _doLoadSingleGrammar(A) {
    const Q = this._options.loadGrammar(A);
    if (Q) {
      const g = typeof this._options.getInjections == "function" ? this._options.getInjections(A) : void 0;
      this._syncRegistry.addGrammar(Q, g);
    }
  }
  /**
   * Adds a rawGrammar.
   */
  addGrammar(A, Q = [], g = 0, E = null) {
    return this._syncRegistry.addGrammar(A, Q), this._grammarForScopeName(A.scopeName, g, E);
  }
  /**
   * Get the grammar for `scopeName`. The grammar must first be created via `loadGrammar` or `addGrammar`.
   */
  _grammarForScopeName(A, Q = 0, g = null, E = null, C = null) {
    return this._syncRegistry.grammarForScopeName(
      A,
      Q,
      g,
      E,
      C
    );
  }
}, mA = qA.NULL;
const ag = [
  "area",
  "base",
  "basefont",
  "bgsound",
  "br",
  "col",
  "command",
  "embed",
  "frame",
  "hr",
  "image",
  "img",
  "input",
  "keygen",
  "link",
  "meta",
  "param",
  "source",
  "track",
  "wbr"
];
class IA {
  /**
   * @param {SchemaType['property']} property
   *   Property.
   * @param {SchemaType['normal']} normal
   *   Normal.
   * @param {Space | undefined} [space]
   *   Space.
   * @returns
   *   Schema.
   */
  constructor(A, Q, g) {
    this.normal = Q, this.property = A, g && (this.space = g);
  }
}
IA.prototype.normal = {};
IA.prototype.property = {};
IA.prototype.space = void 0;
function mB(B, A) {
  const Q = {}, g = {};
  for (const E of B)
    Object.assign(Q, E.property), Object.assign(g, E.normal);
  return new IA(Q, g, A);
}
function bA(B) {
  return B.toLowerCase();
}
class P {
  /**
   * @param {string} property
   *   Property.
   * @param {string} attribute
   *   Attribute.
   * @returns
   *   Info.
   */
  constructor(A, Q) {
    this.attribute = Q, this.property = A;
  }
}
P.prototype.attribute = "";
P.prototype.booleanish = !1;
P.prototype.boolean = !1;
P.prototype.commaOrSpaceSeparated = !1;
P.prototype.commaSeparated = !1;
P.prototype.defined = !1;
P.prototype.mustUseProperty = !1;
P.prototype.number = !1;
P.prototype.overloadedBoolean = !1;
P.prototype.property = "";
P.prototype.spaceSeparated = !1;
P.prototype.space = void 0;
let hg = 0;
const G = T(), H = T(), TA = T(), c = T(), r = T(), X = T(), y = T();
function T() {
  return 2 ** ++hg;
}
const WA = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  boolean: G,
  booleanish: H,
  commaOrSpaceSeparated: y,
  commaSeparated: X,
  number: c,
  overloadedBoolean: TA,
  spaceSeparated: r
}, Symbol.toStringTag, { value: "Module" })), yA = (
  /** @type {ReadonlyArray<keyof typeof types>} */
  Object.keys(WA)
);
class VA extends P {
  /**
   * @constructor
   * @param {string} property
   *   Property.
   * @param {string} attribute
   *   Attribute.
   * @param {number | null | undefined} [mask]
   *   Mask.
   * @param {Space | undefined} [space]
   *   Space.
   * @returns
   *   Info.
   */
  constructor(A, Q, g, E) {
    let C = -1;
    if (super(A, Q), tB(this, "space", E), typeof g == "number")
      for (; ++C < yA.length; ) {
        const I = yA[C];
        tB(this, yA[C], (g & WA[I]) === WA[I]);
      }
  }
}
VA.prototype.defined = !0;
function tB(B, A, Q) {
  Q && (B[A] = Q);
}
function V(B) {
  const A = {}, Q = {};
  for (const [g, E] of Object.entries(B.properties)) {
    const C = new VA(
      g,
      B.transform(B.attributes || {}, g),
      E,
      B.space
    );
    B.mustUseProperty && B.mustUseProperty.includes(g) && (C.mustUseProperty = !0), A[g] = C, Q[bA(g)] = g, Q[bA(C.attribute)] = g;
  }
  return new IA(A, Q, B.space);
}
const bB = V({
  properties: {
    ariaActiveDescendant: null,
    ariaAtomic: H,
    ariaAutoComplete: null,
    ariaBusy: H,
    ariaChecked: H,
    ariaColCount: c,
    ariaColIndex: c,
    ariaColSpan: c,
    ariaControls: r,
    ariaCurrent: null,
    ariaDescribedBy: r,
    ariaDetails: null,
    ariaDisabled: H,
    ariaDropEffect: r,
    ariaErrorMessage: null,
    ariaExpanded: H,
    ariaFlowTo: r,
    ariaGrabbed: H,
    ariaHasPopup: null,
    ariaHidden: H,
    ariaInvalid: null,
    ariaKeyShortcuts: null,
    ariaLabel: null,
    ariaLabelledBy: r,
    ariaLevel: c,
    ariaLive: null,
    ariaModal: H,
    ariaMultiLine: H,
    ariaMultiSelectable: H,
    ariaOrientation: null,
    ariaOwns: r,
    ariaPlaceholder: null,
    ariaPosInSet: c,
    ariaPressed: H,
    ariaReadOnly: H,
    ariaRelevant: null,
    ariaRequired: H,
    ariaRoleDescription: r,
    ariaRowCount: c,
    ariaRowIndex: c,
    ariaRowSpan: c,
    ariaSelected: H,
    ariaSetSize: c,
    ariaSort: null,
    ariaValueMax: c,
    ariaValueMin: c,
    ariaValueNow: c,
    ariaValueText: null,
    role: null
  },
  transform(B, A) {
    return A === "role" ? A : "aria-" + A.slice(4).toLowerCase();
  }
});
function TB(B, A) {
  return A in B ? B[A] : A;
}
function WB(B, A) {
  return TB(B, A.toLowerCase());
}
const Fg = V({
  attributes: {
    acceptcharset: "accept-charset",
    classname: "class",
    htmlfor: "for",
    httpequiv: "http-equiv"
  },
  mustUseProperty: ["checked", "multiple", "muted", "selected"],
  properties: {
    // Standard Properties.
    abbr: null,
    accept: X,
    acceptCharset: r,
    accessKey: r,
    action: null,
    allow: null,
    allowFullScreen: G,
    allowPaymentRequest: G,
    allowUserMedia: G,
    alt: null,
    as: null,
    async: G,
    autoCapitalize: null,
    autoComplete: r,
    autoFocus: G,
    autoPlay: G,
    blocking: r,
    capture: null,
    charSet: null,
    checked: G,
    cite: null,
    className: r,
    cols: c,
    colSpan: null,
    content: null,
    contentEditable: H,
    controls: G,
    controlsList: r,
    coords: c | X,
    crossOrigin: null,
    data: null,
    dateTime: null,
    decoding: null,
    default: G,
    defer: G,
    dir: null,
    dirName: null,
    disabled: G,
    download: TA,
    draggable: H,
    encType: null,
    enterKeyHint: null,
    fetchPriority: null,
    form: null,
    formAction: null,
    formEncType: null,
    formMethod: null,
    formNoValidate: G,
    formTarget: null,
    headers: r,
    height: c,
    hidden: TA,
    high: c,
    href: null,
    hrefLang: null,
    htmlFor: r,
    httpEquiv: r,
    id: null,
    imageSizes: null,
    imageSrcSet: null,
    inert: G,
    inputMode: null,
    integrity: null,
    is: null,
    isMap: G,
    itemId: null,
    itemProp: r,
    itemRef: r,
    itemScope: G,
    itemType: r,
    kind: null,
    label: null,
    lang: null,
    language: null,
    list: null,
    loading: null,
    loop: G,
    low: c,
    manifest: null,
    max: null,
    maxLength: c,
    media: null,
    method: null,
    min: null,
    minLength: c,
    multiple: G,
    muted: G,
    name: null,
    nonce: null,
    noModule: G,
    noValidate: G,
    onAbort: null,
    onAfterPrint: null,
    onAuxClick: null,
    onBeforeMatch: null,
    onBeforePrint: null,
    onBeforeToggle: null,
    onBeforeUnload: null,
    onBlur: null,
    onCancel: null,
    onCanPlay: null,
    onCanPlayThrough: null,
    onChange: null,
    onClick: null,
    onClose: null,
    onContextLost: null,
    onContextMenu: null,
    onContextRestored: null,
    onCopy: null,
    onCueChange: null,
    onCut: null,
    onDblClick: null,
    onDrag: null,
    onDragEnd: null,
    onDragEnter: null,
    onDragExit: null,
    onDragLeave: null,
    onDragOver: null,
    onDragStart: null,
    onDrop: null,
    onDurationChange: null,
    onEmptied: null,
    onEnded: null,
    onError: null,
    onFocus: null,
    onFormData: null,
    onHashChange: null,
    onInput: null,
    onInvalid: null,
    onKeyDown: null,
    onKeyPress: null,
    onKeyUp: null,
    onLanguageChange: null,
    onLoad: null,
    onLoadedData: null,
    onLoadedMetadata: null,
    onLoadEnd: null,
    onLoadStart: null,
    onMessage: null,
    onMessageError: null,
    onMouseDown: null,
    onMouseEnter: null,
    onMouseLeave: null,
    onMouseMove: null,
    onMouseOut: null,
    onMouseOver: null,
    onMouseUp: null,
    onOffline: null,
    onOnline: null,
    onPageHide: null,
    onPageShow: null,
    onPaste: null,
    onPause: null,
    onPlay: null,
    onPlaying: null,
    onPopState: null,
    onProgress: null,
    onRateChange: null,
    onRejectionHandled: null,
    onReset: null,
    onResize: null,
    onScroll: null,
    onScrollEnd: null,
    onSecurityPolicyViolation: null,
    onSeeked: null,
    onSeeking: null,
    onSelect: null,
    onSlotChange: null,
    onStalled: null,
    onStorage: null,
    onSubmit: null,
    onSuspend: null,
    onTimeUpdate: null,
    onToggle: null,
    onUnhandledRejection: null,
    onUnload: null,
    onVolumeChange: null,
    onWaiting: null,
    onWheel: null,
    open: G,
    optimum: c,
    pattern: null,
    ping: r,
    placeholder: null,
    playsInline: G,
    popover: null,
    popoverTarget: null,
    popoverTargetAction: null,
    poster: null,
    preload: null,
    readOnly: G,
    referrerPolicy: null,
    rel: r,
    required: G,
    reversed: G,
    rows: c,
    rowSpan: c,
    sandbox: r,
    scope: null,
    scoped: G,
    seamless: G,
    selected: G,
    shadowRootClonable: G,
    shadowRootDelegatesFocus: G,
    shadowRootMode: null,
    shape: null,
    size: c,
    sizes: null,
    slot: null,
    span: c,
    spellCheck: H,
    src: null,
    srcDoc: null,
    srcLang: null,
    srcSet: null,
    start: c,
    step: null,
    style: null,
    tabIndex: c,
    target: null,
    title: null,
    translate: null,
    type: null,
    typeMustMatch: G,
    useMap: null,
    value: H,
    width: c,
    wrap: null,
    writingSuggestions: null,
    // Legacy.
    // See: https://html.spec.whatwg.org/#other-elements,-attributes-and-apis
    align: null,
    // Several. Use CSS `text-align` instead,
    aLink: null,
    // `<body>`. Use CSS `a:active {color}` instead
    archive: r,
    // `<object>`. List of URIs to archives
    axis: null,
    // `<td>` and `<th>`. Use `scope` on `<th>`
    background: null,
    // `<body>`. Use CSS `background-image` instead
    bgColor: null,
    // `<body>` and table elements. Use CSS `background-color` instead
    border: c,
    // `<table>`. Use CSS `border-width` instead,
    borderColor: null,
    // `<table>`. Use CSS `border-color` instead,
    bottomMargin: c,
    // `<body>`
    cellPadding: null,
    // `<table>`
    cellSpacing: null,
    // `<table>`
    char: null,
    // Several table elements. When `align=char`, sets the character to align on
    charOff: null,
    // Several table elements. When `char`, offsets the alignment
    classId: null,
    // `<object>`
    clear: null,
    // `<br>`. Use CSS `clear` instead
    code: null,
    // `<object>`
    codeBase: null,
    // `<object>`
    codeType: null,
    // `<object>`
    color: null,
    // `<font>` and `<hr>`. Use CSS instead
    compact: G,
    // Lists. Use CSS to reduce space between items instead
    declare: G,
    // `<object>`
    event: null,
    // `<script>`
    face: null,
    // `<font>`. Use CSS instead
    frame: null,
    // `<table>`
    frameBorder: null,
    // `<iframe>`. Use CSS `border` instead
    hSpace: c,
    // `<img>` and `<object>`
    leftMargin: c,
    // `<body>`
    link: null,
    // `<body>`. Use CSS `a:link {color: *}` instead
    longDesc: null,
    // `<frame>`, `<iframe>`, and `<img>`. Use an `<a>`
    lowSrc: null,
    // `<img>`. Use a `<picture>`
    marginHeight: c,
    // `<body>`
    marginWidth: c,
    // `<body>`
    noResize: G,
    // `<frame>`
    noHref: G,
    // `<area>`. Use no href instead of an explicit `nohref`
    noShade: G,
    // `<hr>`. Use background-color and height instead of borders
    noWrap: G,
    // `<td>` and `<th>`
    object: null,
    // `<applet>`
    profile: null,
    // `<head>`
    prompt: null,
    // `<isindex>`
    rev: null,
    // `<link>`
    rightMargin: c,
    // `<body>`
    rules: null,
    // `<table>`
    scheme: null,
    // `<meta>`
    scrolling: H,
    // `<frame>`. Use overflow in the child context
    standby: null,
    // `<object>`
    summary: null,
    // `<table>`
    text: null,
    // `<body>`. Use CSS `color` instead
    topMargin: c,
    // `<body>`
    valueType: null,
    // `<param>`
    version: null,
    // `<html>`. Use a doctype.
    vAlign: null,
    // Several. Use CSS `vertical-align` instead
    vLink: null,
    // `<body>`. Use CSS `a:visited {color}` instead
    vSpace: c,
    // `<img>` and `<object>`
    // Non-standard Properties.
    allowTransparency: null,
    autoCorrect: null,
    autoSave: null,
    disablePictureInPicture: G,
    disableRemotePlayback: G,
    prefix: null,
    property: null,
    results: c,
    security: null,
    unselectable: null
  },
  space: "html",
  transform: WB
}), Gg = V({
  attributes: {
    accentHeight: "accent-height",
    alignmentBaseline: "alignment-baseline",
    arabicForm: "arabic-form",
    baselineShift: "baseline-shift",
    capHeight: "cap-height",
    className: "class",
    clipPath: "clip-path",
    clipRule: "clip-rule",
    colorInterpolation: "color-interpolation",
    colorInterpolationFilters: "color-interpolation-filters",
    colorProfile: "color-profile",
    colorRendering: "color-rendering",
    crossOrigin: "crossorigin",
    dataType: "datatype",
    dominantBaseline: "dominant-baseline",
    enableBackground: "enable-background",
    fillOpacity: "fill-opacity",
    fillRule: "fill-rule",
    floodColor: "flood-color",
    floodOpacity: "flood-opacity",
    fontFamily: "font-family",
    fontSize: "font-size",
    fontSizeAdjust: "font-size-adjust",
    fontStretch: "font-stretch",
    fontStyle: "font-style",
    fontVariant: "font-variant",
    fontWeight: "font-weight",
    glyphName: "glyph-name",
    glyphOrientationHorizontal: "glyph-orientation-horizontal",
    glyphOrientationVertical: "glyph-orientation-vertical",
    hrefLang: "hreflang",
    horizAdvX: "horiz-adv-x",
    horizOriginX: "horiz-origin-x",
    horizOriginY: "horiz-origin-y",
    imageRendering: "image-rendering",
    letterSpacing: "letter-spacing",
    lightingColor: "lighting-color",
    markerEnd: "marker-end",
    markerMid: "marker-mid",
    markerStart: "marker-start",
    navDown: "nav-down",
    navDownLeft: "nav-down-left",
    navDownRight: "nav-down-right",
    navLeft: "nav-left",
    navNext: "nav-next",
    navPrev: "nav-prev",
    navRight: "nav-right",
    navUp: "nav-up",
    navUpLeft: "nav-up-left",
    navUpRight: "nav-up-right",
    onAbort: "onabort",
    onActivate: "onactivate",
    onAfterPrint: "onafterprint",
    onBeforePrint: "onbeforeprint",
    onBegin: "onbegin",
    onCancel: "oncancel",
    onCanPlay: "oncanplay",
    onCanPlayThrough: "oncanplaythrough",
    onChange: "onchange",
    onClick: "onclick",
    onClose: "onclose",
    onCopy: "oncopy",
    onCueChange: "oncuechange",
    onCut: "oncut",
    onDblClick: "ondblclick",
    onDrag: "ondrag",
    onDragEnd: "ondragend",
    onDragEnter: "ondragenter",
    onDragExit: "ondragexit",
    onDragLeave: "ondragleave",
    onDragOver: "ondragover",
    onDragStart: "ondragstart",
    onDrop: "ondrop",
    onDurationChange: "ondurationchange",
    onEmptied: "onemptied",
    onEnd: "onend",
    onEnded: "onended",
    onError: "onerror",
    onFocus: "onfocus",
    onFocusIn: "onfocusin",
    onFocusOut: "onfocusout",
    onHashChange: "onhashchange",
    onInput: "oninput",
    onInvalid: "oninvalid",
    onKeyDown: "onkeydown",
    onKeyPress: "onkeypress",
    onKeyUp: "onkeyup",
    onLoad: "onload",
    onLoadedData: "onloadeddata",
    onLoadedMetadata: "onloadedmetadata",
    onLoadStart: "onloadstart",
    onMessage: "onmessage",
    onMouseDown: "onmousedown",
    onMouseEnter: "onmouseenter",
    onMouseLeave: "onmouseleave",
    onMouseMove: "onmousemove",
    onMouseOut: "onmouseout",
    onMouseOver: "onmouseover",
    onMouseUp: "onmouseup",
    onMouseWheel: "onmousewheel",
    onOffline: "onoffline",
    onOnline: "ononline",
    onPageHide: "onpagehide",
    onPageShow: "onpageshow",
    onPaste: "onpaste",
    onPause: "onpause",
    onPlay: "onplay",
    onPlaying: "onplaying",
    onPopState: "onpopstate",
    onProgress: "onprogress",
    onRateChange: "onratechange",
    onRepeat: "onrepeat",
    onReset: "onreset",
    onResize: "onresize",
    onScroll: "onscroll",
    onSeeked: "onseeked",
    onSeeking: "onseeking",
    onSelect: "onselect",
    onShow: "onshow",
    onStalled: "onstalled",
    onStorage: "onstorage",
    onSubmit: "onsubmit",
    onSuspend: "onsuspend",
    onTimeUpdate: "ontimeupdate",
    onToggle: "ontoggle",
    onUnload: "onunload",
    onVolumeChange: "onvolumechange",
    onWaiting: "onwaiting",
    onZoom: "onzoom",
    overlinePosition: "overline-position",
    overlineThickness: "overline-thickness",
    paintOrder: "paint-order",
    panose1: "panose-1",
    pointerEvents: "pointer-events",
    referrerPolicy: "referrerpolicy",
    renderingIntent: "rendering-intent",
    shapeRendering: "shape-rendering",
    stopColor: "stop-color",
    stopOpacity: "stop-opacity",
    strikethroughPosition: "strikethrough-position",
    strikethroughThickness: "strikethrough-thickness",
    strokeDashArray: "stroke-dasharray",
    strokeDashOffset: "stroke-dashoffset",
    strokeLineCap: "stroke-linecap",
    strokeLineJoin: "stroke-linejoin",
    strokeMiterLimit: "stroke-miterlimit",
    strokeOpacity: "stroke-opacity",
    strokeWidth: "stroke-width",
    tabIndex: "tabindex",
    textAnchor: "text-anchor",
    textDecoration: "text-decoration",
    textRendering: "text-rendering",
    transformOrigin: "transform-origin",
    typeOf: "typeof",
    underlinePosition: "underline-position",
    underlineThickness: "underline-thickness",
    unicodeBidi: "unicode-bidi",
    unicodeRange: "unicode-range",
    unitsPerEm: "units-per-em",
    vAlphabetic: "v-alphabetic",
    vHanging: "v-hanging",
    vIdeographic: "v-ideographic",
    vMathematical: "v-mathematical",
    vectorEffect: "vector-effect",
    vertAdvY: "vert-adv-y",
    vertOriginX: "vert-origin-x",
    vertOriginY: "vert-origin-y",
    wordSpacing: "word-spacing",
    writingMode: "writing-mode",
    xHeight: "x-height",
    // These were camelcased in Tiny. Now lowercased in SVG 2
    playbackOrder: "playbackorder",
    timelineBegin: "timelinebegin"
  },
  properties: {
    about: y,
    accentHeight: c,
    accumulate: null,
    additive: null,
    alignmentBaseline: null,
    alphabetic: c,
    amplitude: c,
    arabicForm: null,
    ascent: c,
    attributeName: null,
    attributeType: null,
    azimuth: c,
    bandwidth: null,
    baselineShift: null,
    baseFrequency: null,
    baseProfile: null,
    bbox: null,
    begin: null,
    bias: c,
    by: null,
    calcMode: null,
    capHeight: c,
    className: r,
    clip: null,
    clipPath: null,
    clipPathUnits: null,
    clipRule: null,
    color: null,
    colorInterpolation: null,
    colorInterpolationFilters: null,
    colorProfile: null,
    colorRendering: null,
    content: null,
    contentScriptType: null,
    contentStyleType: null,
    crossOrigin: null,
    cursor: null,
    cx: null,
    cy: null,
    d: null,
    dataType: null,
    defaultAction: null,
    descent: c,
    diffuseConstant: c,
    direction: null,
    display: null,
    dur: null,
    divisor: c,
    dominantBaseline: null,
    download: G,
    dx: null,
    dy: null,
    edgeMode: null,
    editable: null,
    elevation: c,
    enableBackground: null,
    end: null,
    event: null,
    exponent: c,
    externalResourcesRequired: null,
    fill: null,
    fillOpacity: c,
    fillRule: null,
    filter: null,
    filterRes: null,
    filterUnits: null,
    floodColor: null,
    floodOpacity: null,
    focusable: null,
    focusHighlight: null,
    fontFamily: null,
    fontSize: null,
    fontSizeAdjust: null,
    fontStretch: null,
    fontStyle: null,
    fontVariant: null,
    fontWeight: null,
    format: null,
    fr: null,
    from: null,
    fx: null,
    fy: null,
    g1: X,
    g2: X,
    glyphName: X,
    glyphOrientationHorizontal: null,
    glyphOrientationVertical: null,
    glyphRef: null,
    gradientTransform: null,
    gradientUnits: null,
    handler: null,
    hanging: c,
    hatchContentUnits: null,
    hatchUnits: null,
    height: null,
    href: null,
    hrefLang: null,
    horizAdvX: c,
    horizOriginX: c,
    horizOriginY: c,
    id: null,
    ideographic: c,
    imageRendering: null,
    initialVisibility: null,
    in: null,
    in2: null,
    intercept: c,
    k: c,
    k1: c,
    k2: c,
    k3: c,
    k4: c,
    kernelMatrix: y,
    kernelUnitLength: null,
    keyPoints: null,
    // SEMI_COLON_SEPARATED
    keySplines: null,
    // SEMI_COLON_SEPARATED
    keyTimes: null,
    // SEMI_COLON_SEPARATED
    kerning: null,
    lang: null,
    lengthAdjust: null,
    letterSpacing: null,
    lightingColor: null,
    limitingConeAngle: c,
    local: null,
    markerEnd: null,
    markerMid: null,
    markerStart: null,
    markerHeight: null,
    markerUnits: null,
    markerWidth: null,
    mask: null,
    maskContentUnits: null,
    maskUnits: null,
    mathematical: null,
    max: null,
    media: null,
    mediaCharacterEncoding: null,
    mediaContentEncodings: null,
    mediaSize: c,
    mediaTime: null,
    method: null,
    min: null,
    mode: null,
    name: null,
    navDown: null,
    navDownLeft: null,
    navDownRight: null,
    navLeft: null,
    navNext: null,
    navPrev: null,
    navRight: null,
    navUp: null,
    navUpLeft: null,
    navUpRight: null,
    numOctaves: null,
    observer: null,
    offset: null,
    onAbort: null,
    onActivate: null,
    onAfterPrint: null,
    onBeforePrint: null,
    onBegin: null,
    onCancel: null,
    onCanPlay: null,
    onCanPlayThrough: null,
    onChange: null,
    onClick: null,
    onClose: null,
    onCopy: null,
    onCueChange: null,
    onCut: null,
    onDblClick: null,
    onDrag: null,
    onDragEnd: null,
    onDragEnter: null,
    onDragExit: null,
    onDragLeave: null,
    onDragOver: null,
    onDragStart: null,
    onDrop: null,
    onDurationChange: null,
    onEmptied: null,
    onEnd: null,
    onEnded: null,
    onError: null,
    onFocus: null,
    onFocusIn: null,
    onFocusOut: null,
    onHashChange: null,
    onInput: null,
    onInvalid: null,
    onKeyDown: null,
    onKeyPress: null,
    onKeyUp: null,
    onLoad: null,
    onLoadedData: null,
    onLoadedMetadata: null,
    onLoadStart: null,
    onMessage: null,
    onMouseDown: null,
    onMouseEnter: null,
    onMouseLeave: null,
    onMouseMove: null,
    onMouseOut: null,
    onMouseOver: null,
    onMouseUp: null,
    onMouseWheel: null,
    onOffline: null,
    onOnline: null,
    onPageHide: null,
    onPageShow: null,
    onPaste: null,
    onPause: null,
    onPlay: null,
    onPlaying: null,
    onPopState: null,
    onProgress: null,
    onRateChange: null,
    onRepeat: null,
    onReset: null,
    onResize: null,
    onScroll: null,
    onSeeked: null,
    onSeeking: null,
    onSelect: null,
    onShow: null,
    onStalled: null,
    onStorage: null,
    onSubmit: null,
    onSuspend: null,
    onTimeUpdate: null,
    onToggle: null,
    onUnload: null,
    onVolumeChange: null,
    onWaiting: null,
    onZoom: null,
    opacity: null,
    operator: null,
    order: null,
    orient: null,
    orientation: null,
    origin: null,
    overflow: null,
    overlay: null,
    overlinePosition: c,
    overlineThickness: c,
    paintOrder: null,
    panose1: null,
    path: null,
    pathLength: c,
    patternContentUnits: null,
    patternTransform: null,
    patternUnits: null,
    phase: null,
    ping: r,
    pitch: null,
    playbackOrder: null,
    pointerEvents: null,
    points: null,
    pointsAtX: c,
    pointsAtY: c,
    pointsAtZ: c,
    preserveAlpha: null,
    preserveAspectRatio: null,
    primitiveUnits: null,
    propagate: null,
    property: y,
    r: null,
    radius: null,
    referrerPolicy: null,
    refX: null,
    refY: null,
    rel: y,
    rev: y,
    renderingIntent: null,
    repeatCount: null,
    repeatDur: null,
    requiredExtensions: y,
    requiredFeatures: y,
    requiredFonts: y,
    requiredFormats: y,
    resource: null,
    restart: null,
    result: null,
    rotate: null,
    rx: null,
    ry: null,
    scale: null,
    seed: null,
    shapeRendering: null,
    side: null,
    slope: null,
    snapshotTime: null,
    specularConstant: c,
    specularExponent: c,
    spreadMethod: null,
    spacing: null,
    startOffset: null,
    stdDeviation: null,
    stemh: null,
    stemv: null,
    stitchTiles: null,
    stopColor: null,
    stopOpacity: null,
    strikethroughPosition: c,
    strikethroughThickness: c,
    string: null,
    stroke: null,
    strokeDashArray: y,
    strokeDashOffset: null,
    strokeLineCap: null,
    strokeLineJoin: null,
    strokeMiterLimit: c,
    strokeOpacity: c,
    strokeWidth: null,
    style: null,
    surfaceScale: c,
    syncBehavior: null,
    syncBehaviorDefault: null,
    syncMaster: null,
    syncTolerance: null,
    syncToleranceDefault: null,
    systemLanguage: y,
    tabIndex: c,
    tableValues: null,
    target: null,
    targetX: c,
    targetY: c,
    textAnchor: null,
    textDecoration: null,
    textRendering: null,
    textLength: null,
    timelineBegin: null,
    title: null,
    transformBehavior: null,
    type: null,
    typeOf: y,
    to: null,
    transform: null,
    transformOrigin: null,
    u1: null,
    u2: null,
    underlinePosition: c,
    underlineThickness: c,
    unicode: null,
    unicodeBidi: null,
    unicodeRange: null,
    unitsPerEm: c,
    values: null,
    vAlphabetic: c,
    vMathematical: c,
    vectorEffect: null,
    vHanging: c,
    vIdeographic: c,
    version: null,
    vertAdvY: c,
    vertOriginX: c,
    vertOriginY: c,
    viewBox: null,
    viewTarget: null,
    visibility: null,
    width: null,
    widths: null,
    wordSpacing: null,
    writingMode: null,
    x: null,
    x1: null,
    x2: null,
    xChannelSelector: null,
    xHeight: c,
    y: null,
    y1: null,
    y2: null,
    yChannelSelector: null,
    z: null,
    zoomAndPan: null
  },
  space: "svg",
  transform: TB
}), XB = V({
  properties: {
    xLinkActuate: null,
    xLinkArcRole: null,
    xLinkHref: null,
    xLinkRole: null,
    xLinkShow: null,
    xLinkTitle: null,
    xLinkType: null
  },
  space: "xlink",
  transform(B, A) {
    return "xlink:" + A.slice(5).toLowerCase();
  }
}), ZB = V({
  attributes: { xmlnsxlink: "xmlns:xlink" },
  properties: { xmlnsXLink: null, xmlns: null },
  space: "xmlns",
  transform: WB
}), vB = V({
  properties: { xmlBase: null, xmlLang: null, xmlSpace: null },
  space: "xml",
  transform(B, A) {
    return "xml:" + A.slice(3).toLowerCase();
  }
}), Ng = /[A-Z]/g, cB = /-[a-z]/g, rg = /^data[-\w.:]+$/i;
function Lg(B, A) {
  const Q = bA(A);
  let g = A, E = P;
  if (Q in B.normal)
    return B.property[B.normal[Q]];
  if (Q.length > 4 && Q.slice(0, 4) === "data" && rg.test(A)) {
    if (A.charAt(4) === "-") {
      const C = A.slice(5).replace(cB, Rg);
      g = "data" + C.charAt(0).toUpperCase() + C.slice(1);
    } else {
      const C = A.slice(4);
      if (!cB.test(C)) {
        let I = C.replace(Ng, Hg);
        I.charAt(0) !== "-" && (I = "-" + I), A = "data" + I;
      }
    }
    E = VA;
  }
  return new E(g, A);
}
function Hg(B) {
  return "-" + B.toLowerCase();
}
function Rg(B) {
  return B.charAt(1).toUpperCase();
}
const kg = mB([bB, Fg, XB, ZB, vB], "html"), VB = mB([bB, Gg, XB, ZB, vB], "svg"), nB = {}.hasOwnProperty;
function Kg(B, A) {
  const Q = A || {};
  function g(E, ...C) {
    let I = g.invalid;
    const w = g.handlers;
    if (E && nB.call(E, B)) {
      const D = String(E[B]);
      I = nB.call(w, D) ? w[D] : g.unknown;
    }
    if (I)
      return I.call(this, E, ...C);
  }
  return g.handlers = Q.handlers || {}, g.invalid = Q.invalid, g.unknown = Q.unknown, g;
}
const Ug = /["&'<>`]/g, Yg = /[\uD800-\uDBFF][\uDC00-\uDFFF]/g, lg = (
  // eslint-disable-next-line no-control-regex, unicorn/no-hex-escape
  /[\x01-\t\v\f\x0E-\x1F\x7F\x81\x8D\x8F\x90\x9D\xA0-\uFFFF]/g
), Pg = /[|\\{}()[\]^$+*?.]/g, MB = /* @__PURE__ */ new WeakMap();
function yg(B, A) {
  if (B = B.replace(
    A.subset ? Jg(A.subset) : Ug,
    g
  ), A.subset || A.escapeOnly)
    return B;
  return B.replace(Yg, Q).replace(lg, g);
  function Q(E, C, I) {
    return A.format(
      (E.charCodeAt(0) - 55296) * 1024 + E.charCodeAt(1) - 56320 + 65536,
      I.charCodeAt(C + 2),
      A
    );
  }
  function g(E, C, I) {
    return A.format(
      E.charCodeAt(0),
      I.charCodeAt(C + 1),
      A
    );
  }
}
function Jg(B) {
  let A = MB.get(B);
  return A || (A = ug(B), MB.set(B, A)), A;
}
function ug(B) {
  const A = [];
  let Q = -1;
  for (; ++Q < B.length; )
    A.push(B[Q].replace(Pg, "\\$&"));
  return new RegExp("(?:" + A.join("|") + ")", "g");
}
const Sg = /[\dA-Fa-f]/;
function pg(B, A, Q) {
  const g = "&#x" + B.toString(16).toUpperCase();
  return Q && A && !Sg.test(String.fromCharCode(A)) ? g : g + ";";
}
const fg = /\d/;
function dg(B, A, Q) {
  const g = "&#" + String(B);
  return Q && A && !fg.test(String.fromCharCode(A)) ? g : g + ";";
}
const JA = {
  nbsp: " ",
  iexcl: "¡",
  cent: "¢",
  pound: "£",
  curren: "¤",
  yen: "¥",
  brvbar: "¦",
  sect: "§",
  uml: "¨",
  copy: "©",
  ordf: "ª",
  laquo: "«",
  not: "¬",
  shy: "­",
  reg: "®",
  macr: "¯",
  deg: "°",
  plusmn: "±",
  sup2: "²",
  sup3: "³",
  acute: "´",
  micro: "µ",
  para: "¶",
  middot: "·",
  cedil: "¸",
  sup1: "¹",
  ordm: "º",
  raquo: "»",
  frac14: "¼",
  frac12: "½",
  frac34: "¾",
  iquest: "¿",
  Agrave: "À",
  Aacute: "Á",
  Acirc: "Â",
  Atilde: "Ã",
  Auml: "Ä",
  Aring: "Å",
  AElig: "Æ",
  Ccedil: "Ç",
  Egrave: "È",
  Eacute: "É",
  Ecirc: "Ê",
  Euml: "Ë",
  Igrave: "Ì",
  Iacute: "Í",
  Icirc: "Î",
  Iuml: "Ï",
  ETH: "Ð",
  Ntilde: "Ñ",
  Ograve: "Ò",
  Oacute: "Ó",
  Ocirc: "Ô",
  Otilde: "Õ",
  Ouml: "Ö",
  times: "×",
  Oslash: "Ø",
  Ugrave: "Ù",
  Uacute: "Ú",
  Ucirc: "Û",
  Uuml: "Ü",
  Yacute: "Ý",
  THORN: "Þ",
  szlig: "ß",
  agrave: "à",
  aacute: "á",
  acirc: "â",
  atilde: "ã",
  auml: "ä",
  aring: "å",
  aelig: "æ",
  ccedil: "ç",
  egrave: "è",
  eacute: "é",
  ecirc: "ê",
  euml: "ë",
  igrave: "ì",
  iacute: "í",
  icirc: "î",
  iuml: "ï",
  eth: "ð",
  ntilde: "ñ",
  ograve: "ò",
  oacute: "ó",
  ocirc: "ô",
  otilde: "õ",
  ouml: "ö",
  divide: "÷",
  oslash: "ø",
  ugrave: "ù",
  uacute: "ú",
  ucirc: "û",
  uuml: "ü",
  yacute: "ý",
  thorn: "þ",
  yuml: "ÿ",
  fnof: "ƒ",
  Alpha: "Α",
  Beta: "Β",
  Gamma: "Γ",
  Delta: "Δ",
  Epsilon: "Ε",
  Zeta: "Ζ",
  Eta: "Η",
  Theta: "Θ",
  Iota: "Ι",
  Kappa: "Κ",
  Lambda: "Λ",
  Mu: "Μ",
  Nu: "Ν",
  Xi: "Ξ",
  Omicron: "Ο",
  Pi: "Π",
  Rho: "Ρ",
  Sigma: "Σ",
  Tau: "Τ",
  Upsilon: "Υ",
  Phi: "Φ",
  Chi: "Χ",
  Psi: "Ψ",
  Omega: "Ω",
  alpha: "α",
  beta: "β",
  gamma: "γ",
  delta: "δ",
  epsilon: "ε",
  zeta: "ζ",
  eta: "η",
  theta: "θ",
  iota: "ι",
  kappa: "κ",
  lambda: "λ",
  mu: "μ",
  nu: "ν",
  xi: "ξ",
  omicron: "ο",
  pi: "π",
  rho: "ρ",
  sigmaf: "ς",
  sigma: "σ",
  tau: "τ",
  upsilon: "υ",
  phi: "φ",
  chi: "χ",
  psi: "ψ",
  omega: "ω",
  thetasym: "ϑ",
  upsih: "ϒ",
  piv: "ϖ",
  bull: "•",
  hellip: "…",
  prime: "′",
  Prime: "″",
  oline: "‾",
  frasl: "⁄",
  weierp: "℘",
  image: "ℑ",
  real: "ℜ",
  trade: "™",
  alefsym: "ℵ",
  larr: "←",
  uarr: "↑",
  rarr: "→",
  darr: "↓",
  harr: "↔",
  crarr: "↵",
  lArr: "⇐",
  uArr: "⇑",
  rArr: "⇒",
  dArr: "⇓",
  hArr: "⇔",
  forall: "∀",
  part: "∂",
  exist: "∃",
  empty: "∅",
  nabla: "∇",
  isin: "∈",
  notin: "∉",
  ni: "∋",
  prod: "∏",
  sum: "∑",
  minus: "−",
  lowast: "∗",
  radic: "√",
  prop: "∝",
  infin: "∞",
  ang: "∠",
  and: "∧",
  or: "∨",
  cap: "∩",
  cup: "∪",
  int: "∫",
  there4: "∴",
  sim: "∼",
  cong: "≅",
  asymp: "≈",
  ne: "≠",
  equiv: "≡",
  le: "≤",
  ge: "≥",
  sub: "⊂",
  sup: "⊃",
  nsub: "⊄",
  sube: "⊆",
  supe: "⊇",
  oplus: "⊕",
  otimes: "⊗",
  perp: "⊥",
  sdot: "⋅",
  lceil: "⌈",
  rceil: "⌉",
  lfloor: "⌊",
  rfloor: "⌋",
  lang: "〈",
  rang: "〉",
  loz: "◊",
  spades: "♠",
  clubs: "♣",
  hearts: "♥",
  diams: "♦",
  quot: '"',
  amp: "&",
  lt: "<",
  gt: ">",
  OElig: "Œ",
  oelig: "œ",
  Scaron: "Š",
  scaron: "š",
  Yuml: "Ÿ",
  circ: "ˆ",
  tilde: "˜",
  ensp: " ",
  emsp: " ",
  thinsp: " ",
  zwnj: "‌",
  zwj: "‍",
  lrm: "‎",
  rlm: "‏",
  ndash: "–",
  mdash: "—",
  lsquo: "‘",
  rsquo: "’",
  sbquo: "‚",
  ldquo: "“",
  rdquo: "”",
  bdquo: "„",
  dagger: "†",
  Dagger: "‡",
  permil: "‰",
  lsaquo: "‹",
  rsaquo: "›",
  euro: "€"
}, Og = [
  "cent",
  "copy",
  "divide",
  "gt",
  "lt",
  "not",
  "para",
  "times"
], zB = {}.hasOwnProperty, XA = {};
let oA;
for (oA in JA)
  zB.call(JA, oA) && (XA[JA[oA]] = oA);
const xg = /[^\dA-Za-z]/;
function qg(B, A, Q, g) {
  const E = String.fromCharCode(B);
  if (zB.call(XA, E)) {
    const C = XA[E], I = "&" + C;
    return Q && eQ.includes(C) && !Og.includes(C) && (!g || A && A !== 61 && xg.test(String.fromCharCode(A))) ? I : I + ";";
  }
  return "";
}
function mg(B, A, Q) {
  let g = pg(B, A, Q.omitOptionalSemicolons), E;
  if ((Q.useNamedReferences || Q.useShortestReferences) && (E = qg(
    B,
    A,
    Q.omitOptionalSemicolons,
    Q.attribute
  )), (Q.useShortestReferences || !E) && Q.useShortestReferences) {
    const C = dg(B, A, Q.omitOptionalSemicolons);
    C.length < g.length && (g = C);
  }
  return E && (!Q.useShortestReferences || E.length < g.length) ? E : g;
}
function Z(B, A) {
  return yg(B, Object.assign({ format: mg }, A));
}
const bg = /^>|^->|<!--|-->|--!>|<!-$/g, Tg = [">"], Wg = ["<", ">"];
function Xg(B, A, Q, g) {
  return g.settings.bogusComments ? "<?" + Z(
    B.value,
    Object.assign({}, g.settings.characterReferences, {
      subset: Tg
    })
  ) + ">" : "<!--" + B.value.replace(bg, E) + "-->";
  function E(C) {
    return Z(
      C,
      Object.assign({}, g.settings.characterReferences, {
        subset: Wg
      })
    );
  }
}
function Zg(B, A, Q, g) {
  return "<!" + (g.settings.upperDoctype ? "DOCTYPE" : "doctype") + (g.settings.tightDoctype ? "" : " ") + "html>";
}
function aB(B, A) {
  const Q = String(B);
  if (typeof A != "string")
    throw new TypeError("Expected character");
  let g = 0, E = Q.indexOf(A);
  for (; E !== -1; )
    g++, E = Q.indexOf(A, E + A.length);
  return g;
}
function hC(B) {
  const A = [], Q = String(B || "");
  let g = Q.indexOf(","), E = 0, C = !1;
  for (; !C; ) {
    g === -1 && (g = Q.length, C = !0);
    const I = Q.slice(E, g).trim();
    (I || !C) && A.push(I), E = g + 1, g = Q.indexOf(",", E);
  }
  return A;
}
function vg(B, A) {
  const Q = A || {};
  return (B[B.length - 1] === "" ? [...B, ""] : B).join(
    (Q.padRight ? " " : "") + "," + (Q.padLeft === !1 ? "" : " ")
  ).trim();
}
function FC(B) {
  const A = String(B || "").trim();
  return A ? A.split(/[ \t\n\r\f]+/g) : [];
}
function Vg(B) {
  return B.join(" ").trim();
}
const zg = /[ \t\n\f\r]/g;
function zA(B) {
  return typeof B == "object" ? B.type === "text" ? hB(B.value) : !1 : hB(B);
}
function hB(B) {
  return B.replace(zg, "") === "";
}
const K = _B(1), jB = _B(-1), jg = [];
function _B(B) {
  return A;
  function A(Q, g, E) {
    const C = Q ? Q.children : jg;
    let I = (g || 0) + B, w = C[I];
    if (!E)
      for (; w && zA(w); )
        I += B, w = C[I];
    return w;
  }
}
const _g = {}.hasOwnProperty;
function $B(B) {
  return A;
  function A(Q, g, E) {
    return _g.call(B, Q.tagName) && B[Q.tagName](Q, g, E);
  }
}
const jA = $B({
  body: AE,
  caption: uA,
  colgroup: uA,
  dd: EE,
  dt: gE,
  head: uA,
  html: $g,
  li: QE,
  optgroup: CE,
  option: IE,
  p: BE,
  rp: FB,
  rt: FB,
  tbody: wE,
  td: GB,
  tfoot: oE,
  th: GB,
  thead: DE,
  tr: sE
});
function uA(B, A, Q) {
  const g = K(Q, A, !0);
  return !g || g.type !== "comment" && !(g.type === "text" && zA(g.value.charAt(0)));
}
function $g(B, A, Q) {
  const g = K(Q, A);
  return !g || g.type !== "comment";
}
function AE(B, A, Q) {
  const g = K(Q, A);
  return !g || g.type !== "comment";
}
function BE(B, A, Q) {
  const g = K(Q, A);
  return g ? g.type === "element" && (g.tagName === "address" || g.tagName === "article" || g.tagName === "aside" || g.tagName === "blockquote" || g.tagName === "details" || g.tagName === "div" || g.tagName === "dl" || g.tagName === "fieldset" || g.tagName === "figcaption" || g.tagName === "figure" || g.tagName === "footer" || g.tagName === "form" || g.tagName === "h1" || g.tagName === "h2" || g.tagName === "h3" || g.tagName === "h4" || g.tagName === "h5" || g.tagName === "h6" || g.tagName === "header" || g.tagName === "hgroup" || g.tagName === "hr" || g.tagName === "main" || g.tagName === "menu" || g.tagName === "nav" || g.tagName === "ol" || g.tagName === "p" || g.tagName === "pre" || g.tagName === "section" || g.tagName === "table" || g.tagName === "ul") : !Q || // Confusing parent.
  !(Q.type === "element" && (Q.tagName === "a" || Q.tagName === "audio" || Q.tagName === "del" || Q.tagName === "ins" || Q.tagName === "map" || Q.tagName === "noscript" || Q.tagName === "video"));
}
function QE(B, A, Q) {
  const g = K(Q, A);
  return !g || g.type === "element" && g.tagName === "li";
}
function gE(B, A, Q) {
  const g = K(Q, A);
  return !!(g && g.type === "element" && (g.tagName === "dt" || g.tagName === "dd"));
}
function EE(B, A, Q) {
  const g = K(Q, A);
  return !g || g.type === "element" && (g.tagName === "dt" || g.tagName === "dd");
}
function FB(B, A, Q) {
  const g = K(Q, A);
  return !g || g.type === "element" && (g.tagName === "rp" || g.tagName === "rt");
}
function CE(B, A, Q) {
  const g = K(Q, A);
  return !g || g.type === "element" && g.tagName === "optgroup";
}
function IE(B, A, Q) {
  const g = K(Q, A);
  return !g || g.type === "element" && (g.tagName === "option" || g.tagName === "optgroup");
}
function DE(B, A, Q) {
  const g = K(Q, A);
  return !!(g && g.type === "element" && (g.tagName === "tbody" || g.tagName === "tfoot"));
}
function wE(B, A, Q) {
  const g = K(Q, A);
  return !g || g.type === "element" && (g.tagName === "tbody" || g.tagName === "tfoot");
}
function oE(B, A, Q) {
  return !K(Q, A);
}
function sE(B, A, Q) {
  const g = K(Q, A);
  return !g || g.type === "element" && g.tagName === "tr";
}
function GB(B, A, Q) {
  const g = K(Q, A);
  return !g || g.type === "element" && (g.tagName === "td" || g.tagName === "th");
}
const eE = $B({
  body: cE,
  colgroup: nE,
  head: tE,
  html: iE,
  tbody: ME
});
function iE(B) {
  const A = K(B, -1);
  return !A || A.type !== "comment";
}
function tE(B) {
  const A = /* @__PURE__ */ new Set();
  for (const g of B.children)
    if (g.type === "element" && (g.tagName === "base" || g.tagName === "title")) {
      if (A.has(g.tagName)) return !1;
      A.add(g.tagName);
    }
  const Q = B.children[0];
  return !Q || Q.type === "element";
}
function cE(B) {
  const A = K(B, -1, !0);
  return !A || A.type !== "comment" && !(A.type === "text" && zA(A.value.charAt(0))) && !(A.type === "element" && (A.tagName === "meta" || A.tagName === "link" || A.tagName === "script" || A.tagName === "style" || A.tagName === "template"));
}
function nE(B, A, Q) {
  const g = jB(Q, A), E = K(B, -1, !0);
  return Q && g && g.type === "element" && g.tagName === "colgroup" && jA(g, Q.children.indexOf(g), Q) ? !1 : !!(E && E.type === "element" && E.tagName === "col");
}
function ME(B, A, Q) {
  const g = jB(Q, A), E = K(B, -1);
  return Q && g && g.type === "element" && (g.tagName === "thead" || g.tagName === "tbody") && jA(g, Q.children.indexOf(g), Q) ? !1 : !!(E && E.type === "element" && E.tagName === "tr");
}
const sA = {
  // See: <https://html.spec.whatwg.org/#attribute-name-state>.
  name: [
    [`	
\f\r &/=>`.split(""), `	
\f\r "&'/=>\``.split("")],
    [`\0	
\f\r "&'/<=>`.split(""), `\0	
\f\r "&'/<=>\``.split("")]
  ],
  // See: <https://html.spec.whatwg.org/#attribute-value-(unquoted)-state>.
  unquoted: [
    [`	
\f\r &>`.split(""), `\0	
\f\r "&'<=>\``.split("")],
    [`\0	
\f\r "&'<=>\``.split(""), `\0	
\f\r "&'<=>\``.split("")]
  ],
  // See: <https://html.spec.whatwg.org/#attribute-value-(single-quoted)-state>.
  single: [
    ["&'".split(""), "\"&'`".split("")],
    ["\0&'".split(""), "\0\"&'`".split("")]
  ],
  // See: <https://html.spec.whatwg.org/#attribute-value-(double-quoted)-state>.
  double: [
    ['"&'.split(""), "\"&'`".split("")],
    ['\0"&'.split(""), "\0\"&'`".split("")]
  ]
};
function aE(B, A, Q, g) {
  const E = g.schema, C = E.space === "svg" ? !1 : g.settings.omitOptionalTags;
  let I = E.space === "svg" ? g.settings.closeEmptyElements : g.settings.voids.includes(B.tagName.toLowerCase());
  const w = [];
  let D;
  E.space === "html" && B.tagName === "svg" && (g.schema = VB);
  const o = hE(g, B.properties), s = g.all(
    E.space === "html" && B.tagName === "template" ? B.content : B
  );
  return g.schema = E, s && (I = !1), (o || !C || !eE(B, A, Q)) && (w.push("<", B.tagName, o ? " " + o : ""), I && (E.space === "svg" || g.settings.closeSelfClosing) && (D = o.charAt(o.length - 1), (!g.settings.tightSelfClosing || D === "/" || D && D !== '"' && D !== "'") && w.push(" "), w.push("/")), w.push(">")), w.push(s), !I && (!C || !jA(B, A, Q)) && w.push("</" + B.tagName + ">"), w.join("");
}
function hE(B, A) {
  const Q = [];
  let g = -1, E;
  if (A) {
    for (E in A)
      if (A[E] !== null && A[E] !== void 0) {
        const C = FE(B, E, A[E]);
        C && Q.push(C);
      }
  }
  for (; ++g < Q.length; ) {
    const C = B.settings.tightAttributes ? Q[g].charAt(Q[g].length - 1) : void 0;
    g !== Q.length - 1 && C !== '"' && C !== "'" && (Q[g] += " ");
  }
  return Q.join("");
}
function FE(B, A, Q) {
  const g = Lg(B.schema, A), E = B.settings.allowParseErrors && B.schema.space === "html" ? 0 : 1, C = B.settings.allowDangerousCharacters ? 0 : 1;
  let I = B.quote, w;
  if (g.overloadedBoolean && (Q === g.attribute || Q === "") ? Q = !0 : (g.boolean || g.overloadedBoolean) && (typeof Q != "string" || Q === g.attribute || Q === "") && (Q = !!Q), Q == null || Q === !1 || typeof Q == "number" && Number.isNaN(Q))
    return "";
  const D = Z(
    g.attribute,
    Object.assign({}, B.settings.characterReferences, {
      // Always encode without parse errors in non-HTML.
      subset: sA.name[E][C]
    })
  );
  return Q === !0 || (Q = Array.isArray(Q) ? (g.commaSeparated ? vg : Vg)(Q, {
    padLeft: !B.settings.tightCommaSeparatedLists
  }) : String(Q), B.settings.collapseEmptyAttributes && !Q) ? D : (B.settings.preferUnquoted && (w = Z(
    Q,
    Object.assign({}, B.settings.characterReferences, {
      attribute: !0,
      subset: sA.unquoted[E][C]
    })
  )), w !== Q && (B.settings.quoteSmart && aB(Q, I) > aB(Q, B.alternative) && (I = B.alternative), w = I + Z(
    Q,
    Object.assign({}, B.settings.characterReferences, {
      // Always encode without parse errors in non-HTML.
      subset: (I === "'" ? sA.single : sA.double)[E][C],
      attribute: !0
    })
  ) + I), D + (w && "=" + w));
}
const GE = ["<", "&"];
function AQ(B, A, Q, g) {
  return Q && Q.type === "element" && (Q.tagName === "script" || Q.tagName === "style") ? B.value : Z(
    B.value,
    Object.assign({}, g.settings.characterReferences, {
      subset: GE
    })
  );
}
function NE(B, A, Q, g) {
  return g.settings.allowDangerousHtml ? B.value : AQ(B, A, Q, g);
}
function rE(B, A, Q, g) {
  return g.all(B);
}
const LE = Kg("type", {
  invalid: HE,
  unknown: RE,
  handlers: { comment: Xg, doctype: Zg, element: aE, raw: NE, root: rE, text: AQ }
});
function HE(B) {
  throw new Error("Expected node, not `" + B + "`");
}
function RE(B) {
  const A = (
    /** @type {Nodes} */
    B
  );
  throw new Error("Cannot compile unknown node `" + A.type + "`");
}
const kE = {}, KE = {}, UE = [];
function YE(B, A) {
  const Q = A || kE, g = Q.quote || '"', E = g === '"' ? "'" : '"';
  if (g !== '"' && g !== "'")
    throw new Error("Invalid quote `" + g + "`, expected `'` or `\"`");
  return {
    one: lE,
    all: PE,
    settings: {
      omitOptionalTags: Q.omitOptionalTags || !1,
      allowParseErrors: Q.allowParseErrors || !1,
      allowDangerousCharacters: Q.allowDangerousCharacters || !1,
      quoteSmart: Q.quoteSmart || !1,
      preferUnquoted: Q.preferUnquoted || !1,
      tightAttributes: Q.tightAttributes || !1,
      upperDoctype: Q.upperDoctype || !1,
      tightDoctype: Q.tightDoctype || !1,
      bogusComments: Q.bogusComments || !1,
      tightCommaSeparatedLists: Q.tightCommaSeparatedLists || !1,
      tightSelfClosing: Q.tightSelfClosing || !1,
      collapseEmptyAttributes: Q.collapseEmptyAttributes || !1,
      allowDangerousHtml: Q.allowDangerousHtml || !1,
      voids: Q.voids || ag,
      characterReferences: Q.characterReferences || KE,
      closeSelfClosing: Q.closeSelfClosing || !1,
      closeEmptyElements: Q.closeEmptyElements || !1
    },
    schema: Q.space === "svg" ? VB : kg,
    quote: g,
    alternative: E
  }.one(
    Array.isArray(B) ? { type: "root", children: B } : B,
    void 0,
    void 0
  );
}
function lE(B, A, Q) {
  return LE(B, A, Q, this);
}
function PE(B) {
  const A = [], Q = B && B.children || UE;
  let g = -1;
  for (; ++g < Q.length; )
    A[g] = this.one(Q[g], g, B);
  return A.join("");
}
function yE(B) {
  return Array.isArray(B) ? B : [B];
}
function HA(B, A = !1) {
  const Q = B.split(/(\r?\n)/g);
  let g = 0;
  const E = [];
  for (let C = 0; C < Q.length; C += 2) {
    const I = A ? Q[C] + (Q[C + 1] || "") : Q[C];
    E.push([I, g]), g += Q[C].length, g += Q[C + 1]?.length || 0;
  }
  return E;
}
function _A(B) {
  return !B || ["plaintext", "txt", "text", "plain"].includes(B);
}
function JE(B) {
  return B === "ansi" || _A(B);
}
function $A(B) {
  return B === "none";
}
function uE(B) {
  return $A(B);
}
function BQ(B, A) {
  if (!A)
    return B;
  B.properties ||= {}, B.properties.class ||= [], typeof B.properties.class == "string" && (B.properties.class = B.properties.class.split(/\s+/g)), Array.isArray(B.properties.class) || (B.properties.class = []);
  const Q = Array.isArray(A) ? A : A.split(/\s+/g);
  for (const g of Q)
    g && !B.properties.class.includes(g) && B.properties.class.push(g);
  return B;
}
function SE(B, A) {
  let Q = 0;
  const g = [];
  for (const E of A)
    E > Q && g.push({
      ...B,
      content: B.content.slice(Q, E),
      offset: B.offset + Q
    }), Q = E;
  return Q < B.content.length && g.push({
    ...B,
    content: B.content.slice(Q),
    offset: B.offset + Q
  }), g;
}
function pE(B, A) {
  const Q = Array.from(A instanceof Set ? A : new Set(A)).sort((g, E) => g - E);
  return Q.length ? B.map((g) => g.flatMap((E) => {
    const C = Q.filter((I) => E.offset < I && I < E.offset + E.content.length).map((I) => I - E.offset).sort((I, w) => I - w);
    return C.length ? SE(E, C) : E;
  })) : B;
}
async function QQ(B) {
  return Promise.resolve(typeof B == "function" ? B() : B).then((A) => A.default || A);
}
function hA(B, A) {
  const Q = typeof B == "string" ? {} : { ...B.colorReplacements }, g = typeof B == "string" ? B : B.name;
  for (const [E, C] of Object.entries(A?.colorReplacements || {}))
    typeof C == "string" ? Q[E] = C : E === g && Object.assign(Q, C);
  return Q;
}
function q(B, A) {
  return B && (A?.[B?.toLowerCase()] || B);
}
function gQ(B) {
  const A = {};
  return B.color && (A.color = B.color), B.bgColor && (A["background-color"] = B.bgColor), B.fontStyle && (B.fontStyle & d.Italic && (A["font-style"] = "italic"), B.fontStyle & d.Bold && (A["font-weight"] = "bold"), B.fontStyle & d.Underline && (A["text-decoration"] = "underline")), A;
}
function fE(B) {
  return typeof B == "string" ? B : Object.entries(B).map(([A, Q]) => `${A}:${Q}`).join(";");
}
function dE(B) {
  const A = HA(B, !0).map(([E]) => E);
  function Q(E) {
    if (E === B.length)
      return {
        line: A.length - 1,
        character: A[A.length - 1].length
      };
    let C = E, I = 0;
    for (const w of A) {
      if (C < w.length)
        break;
      C -= w.length, I++;
    }
    return { line: I, character: C };
  }
  function g(E, C) {
    let I = 0;
    for (let w = 0; w < E; w++)
      I += A[w].length;
    return I += C, I;
  }
  return {
    lines: A,
    indexToPos: Q,
    posToIndex: g
  };
}
class l extends Error {
  constructor(A) {
    super(A), this.name = "ShikiError";
  }
}
const EQ = /* @__PURE__ */ new WeakMap();
function RA(B, A) {
  EQ.set(B, A);
}
function gA(B) {
  return EQ.get(B);
}
class z {
  /**
   * Theme to Stack mapping
   */
  _stacks = {};
  lang;
  get themes() {
    return Object.keys(this._stacks);
  }
  get theme() {
    return this.themes[0];
  }
  get _stack() {
    return this._stacks[this.theme];
  }
  /**
   * Static method to create a initial grammar state.
   */
  static initial(A, Q) {
    return new z(
      Object.fromEntries(yE(Q).map((g) => [g, mA])),
      A
    );
  }
  constructor(...A) {
    if (A.length === 2) {
      const [Q, g] = A;
      this.lang = g, this._stacks = Q;
    } else {
      const [Q, g, E] = A;
      this.lang = g, this._stacks = { [E]: Q };
    }
  }
  /**
   * Get the internal stack object.
   * @internal
   */
  getInternalStack(A = this.theme) {
    return this._stacks[A];
  }
  /**
   * @deprecated use `getScopes` instead
   */
  get scopes() {
    return NB(this._stacks[this.theme]);
  }
  getScopes(A = this.theme) {
    return NB(this._stacks[A]);
  }
  toJSON() {
    return {
      lang: this.lang,
      theme: this.theme,
      themes: this.themes,
      scopes: this.scopes
    };
  }
}
function NB(B) {
  const A = [], Q = /* @__PURE__ */ new Set();
  function g(E) {
    if (Q.has(E))
      return;
    Q.add(E);
    const C = E?.nameScopesList?.scopeName;
    C && A.push(C), E.parent && g(E.parent);
  }
  return g(B), A;
}
function OE(B, A) {
  if (!(B instanceof z))
    throw new l("Invalid grammar state");
  return B.getInternalStack(A);
}
function xE() {
  const B = /* @__PURE__ */ new WeakMap();
  function A(Q) {
    if (!B.has(Q.meta)) {
      let g = function(I) {
        if (typeof I == "number") {
          if (I < 0 || I > Q.source.length)
            throw new l(`Invalid decoration offset: ${I}. Code length: ${Q.source.length}`);
          return {
            ...E.indexToPos(I),
            offset: I
          };
        } else {
          const w = E.lines[I.line];
          if (w === void 0)
            throw new l(`Invalid decoration position ${JSON.stringify(I)}. Lines length: ${E.lines.length}`);
          if (I.character < 0 || I.character > w.length)
            throw new l(`Invalid decoration position ${JSON.stringify(I)}. Line ${I.line} length: ${w.length}`);
          return {
            ...I,
            offset: E.posToIndex(I.line, I.character)
          };
        }
      };
      const E = dE(Q.source), C = (Q.options.decorations || []).map((I) => ({
        ...I,
        start: g(I.start),
        end: g(I.end)
      }));
      qE(C), B.set(Q.meta, {
        decorations: C,
        converter: E,
        source: Q.source
      });
    }
    return B.get(Q.meta);
  }
  return {
    name: "shiki:decorations",
    tokens(Q) {
      if (!this.options.decorations?.length)
        return;
      const E = A(this).decorations.flatMap((I) => [I.start.offset, I.end.offset]);
      return pE(Q, E);
    },
    code(Q) {
      if (!this.options.decorations?.length)
        return;
      const g = A(this), E = Array.from(Q.children).filter((s) => s.type === "element" && s.tagName === "span");
      if (E.length !== g.converter.lines.length)
        throw new l(`Number of lines in code element (${E.length}) does not match the number of lines in the source (${g.converter.lines.length}). Failed to apply decorations.`);
      function C(s, e, n, i) {
        const t = E[s];
        let N = "", M = -1, h = -1;
        if (e === 0 && (M = 0), n === 0 && (h = 0), n === Number.POSITIVE_INFINITY && (h = t.children.length), M === -1 || h === -1)
          for (let F = 0; F < t.children.length; F++)
            N += CQ(t.children[F]), M === -1 && N.length === e && (M = F + 1), h === -1 && N.length === n && (h = F + 1);
        if (M === -1)
          throw new l(`Failed to find start index for decoration ${JSON.stringify(i.start)}`);
        if (h === -1)
          throw new l(`Failed to find end index for decoration ${JSON.stringify(i.end)}`);
        const a = t.children.slice(M, h);
        if (!i.alwaysWrap && a.length === t.children.length)
          w(t, i, "line");
        else if (!i.alwaysWrap && a.length === 1 && a[0].type === "element")
          w(a[0], i, "token");
        else {
          const F = {
            type: "element",
            tagName: "span",
            properties: {},
            children: a
          };
          w(F, i, "wrapper"), t.children.splice(M, a.length, F);
        }
      }
      function I(s, e) {
        E[s] = w(E[s], e, "line");
      }
      function w(s, e, n) {
        const i = e.properties || {}, t = e.transform || ((N) => N);
        return s.tagName = e.tagName || "span", s.properties = {
          ...s.properties,
          ...i,
          class: s.properties.class
        }, e.properties?.class && BQ(s, e.properties.class), s = t(s, n) || s, s;
      }
      const D = [], o = g.decorations.sort((s, e) => e.start.offset - s.start.offset);
      for (const s of o) {
        const { start: e, end: n } = s;
        if (e.line === n.line)
          C(e.line, e.character, n.character, s);
        else if (e.line < n.line) {
          C(e.line, e.character, Number.POSITIVE_INFINITY, s);
          for (let i = e.line + 1; i < n.line; i++)
            D.unshift(() => I(i, s));
          C(n.line, 0, n.character, s);
        }
      }
      D.forEach((s) => s());
    }
  };
}
function qE(B) {
  for (let A = 0; A < B.length; A++) {
    const Q = B[A];
    if (Q.start.offset > Q.end.offset)
      throw new l(`Invalid decoration range: ${JSON.stringify(Q.start)} - ${JSON.stringify(Q.end)}`);
    for (let g = A + 1; g < B.length; g++) {
      const E = B[g], C = Q.start.offset < E.start.offset && E.start.offset < Q.end.offset, I = Q.start.offset < E.end.offset && E.end.offset < Q.end.offset, w = E.start.offset < Q.start.offset && Q.start.offset < E.end.offset, D = E.start.offset < Q.end.offset && Q.end.offset < E.end.offset;
      if (C || I || w || D) {
        if (I && I || w && D)
          continue;
        throw new l(`Decorations ${JSON.stringify(Q.start)} and ${JSON.stringify(E.start)} intersect.`);
      }
    }
  }
}
function CQ(B) {
  return B.type === "text" ? B.value : B.type === "element" ? B.children.map(CQ).join("") : "";
}
const mE = [
  /* @__PURE__ */ xE()
];
function FA(B) {
  return [
    ...B.transformers || [],
    ...mE
  ];
}
var m = [
  "black",
  "red",
  "green",
  "yellow",
  "blue",
  "magenta",
  "cyan",
  "white",
  "brightBlack",
  "brightRed",
  "brightGreen",
  "brightYellow",
  "brightBlue",
  "brightMagenta",
  "brightCyan",
  "brightWhite"
], SA = {
  1: "bold",
  2: "dim",
  3: "italic",
  4: "underline",
  7: "reverse",
  9: "strikethrough"
};
function bE(B, A) {
  const Q = B.indexOf("\x1B[", A);
  if (Q !== -1) {
    const g = B.indexOf("m", Q);
    return {
      sequence: B.substring(Q + 2, g).split(";"),
      startPosition: Q,
      position: g + 1
    };
  }
  return {
    position: B.length
  };
}
function rB(B, A) {
  let Q = 1;
  const g = B[A + Q++];
  let E;
  if (g === "2") {
    const C = [
      B[A + Q++],
      B[A + Q++],
      B[A + Q]
    ].map((I) => Number.parseInt(I));
    C.length === 3 && !C.some((I) => Number.isNaN(I)) && (E = {
      type: "rgb",
      rgb: C
    });
  } else if (g === "5") {
    const C = Number.parseInt(B[A + Q]);
    Number.isNaN(C) || (E = { type: "table", index: Number(C) });
  }
  return [Q, E];
}
function TE(B) {
  const A = [];
  for (let Q = 0; Q < B.length; Q++) {
    const g = B[Q], E = Number.parseInt(g);
    if (!Number.isNaN(E))
      if (E === 0)
        A.push({ type: "resetAll" });
      else if (E <= 9)
        SA[E] && A.push({
          type: "setDecoration",
          value: SA[E]
        });
      else if (E <= 29) {
        const C = SA[E - 20];
        C && A.push({
          type: "resetDecoration",
          value: C
        });
      } else if (E <= 37)
        A.push({
          type: "setForegroundColor",
          value: { type: "named", name: m[E - 30] }
        });
      else if (E === 38) {
        const [C, I] = rB(B, Q);
        I && A.push({
          type: "setForegroundColor",
          value: I
        }), Q += C;
      } else if (E === 39)
        A.push({
          type: "resetForegroundColor"
        });
      else if (E <= 47)
        A.push({
          type: "setBackgroundColor",
          value: { type: "named", name: m[E - 40] }
        });
      else if (E === 48) {
        const [C, I] = rB(B, Q);
        I && A.push({
          type: "setBackgroundColor",
          value: I
        }), Q += C;
      } else E === 49 ? A.push({
        type: "resetBackgroundColor"
      }) : E >= 90 && E <= 97 ? A.push({
        type: "setForegroundColor",
        value: { type: "named", name: m[E - 90 + 8] }
      }) : E >= 100 && E <= 107 && A.push({
        type: "setBackgroundColor",
        value: { type: "named", name: m[E - 100 + 8] }
      });
  }
  return A;
}
function WE() {
  let B = null, A = null, Q = /* @__PURE__ */ new Set();
  return {
    parse(g) {
      const E = [];
      let C = 0;
      do {
        const I = bE(g, C), w = I.sequence ? g.substring(C, I.startPosition) : g.substring(C);
        if (w.length > 0 && E.push({
          value: w,
          foreground: B,
          background: A,
          decorations: new Set(Q)
        }), I.sequence) {
          const D = TE(I.sequence);
          for (const o of D)
            o.type === "resetAll" ? (B = null, A = null, Q.clear()) : o.type === "resetForegroundColor" ? B = null : o.type === "resetBackgroundColor" ? A = null : o.type === "resetDecoration" && Q.delete(o.value);
          for (const o of D)
            o.type === "setForegroundColor" ? B = o.value : o.type === "setBackgroundColor" ? A = o.value : o.type === "setDecoration" && Q.add(o.value);
        }
        C = I.position;
      } while (C < g.length);
      return E;
    }
  };
}
var XE = {
  black: "#000000",
  red: "#bb0000",
  green: "#00bb00",
  yellow: "#bbbb00",
  blue: "#0000bb",
  magenta: "#ff00ff",
  cyan: "#00bbbb",
  white: "#eeeeee",
  brightBlack: "#555555",
  brightRed: "#ff5555",
  brightGreen: "#00ff00",
  brightYellow: "#ffff55",
  brightBlue: "#5555ff",
  brightMagenta: "#ff55ff",
  brightCyan: "#55ffff",
  brightWhite: "#ffffff"
};
function ZE(B = XE) {
  function A(w) {
    return B[w];
  }
  function Q(w) {
    return `#${w.map((D) => Math.max(0, Math.min(D, 255)).toString(16).padStart(2, "0")).join("")}`;
  }
  let g;
  function E() {
    if (g)
      return g;
    g = [];
    for (let o = 0; o < m.length; o++)
      g.push(A(m[o]));
    let w = [0, 95, 135, 175, 215, 255];
    for (let o = 0; o < 6; o++)
      for (let s = 0; s < 6; s++)
        for (let e = 0; e < 6; e++)
          g.push(Q([w[o], w[s], w[e]]));
    let D = 8;
    for (let o = 0; o < 24; o++, D += 10)
      g.push(Q([D, D, D]));
    return g;
  }
  function C(w) {
    return E()[w];
  }
  function I(w) {
    switch (w.type) {
      case "named":
        return A(w.name);
      case "rgb":
        return Q(w.rgb);
      case "table":
        return C(w.index);
    }
  }
  return {
    value: I
  };
}
function vE(B, A, Q) {
  const g = hA(B, Q), E = HA(A), C = ZE(
    Object.fromEntries(
      m.map((w) => [
        w,
        B.colors?.[`terminal.ansi${w[0].toUpperCase()}${w.substring(1)}`]
      ])
    )
  ), I = WE();
  return E.map(
    (w) => I.parse(w[0]).map((D) => {
      let o, s;
      D.decorations.has("reverse") ? (o = D.background ? C.value(D.background) : B.bg, s = D.foreground ? C.value(D.foreground) : B.fg) : (o = D.foreground ? C.value(D.foreground) : B.fg, s = D.background ? C.value(D.background) : void 0), o = q(o, g), s = q(s, g), D.decorations.has("dim") && (o = VE(o));
      let e = d.None;
      return D.decorations.has("bold") && (e |= d.Bold), D.decorations.has("italic") && (e |= d.Italic), D.decorations.has("underline") && (e |= d.Underline), {
        content: D.value,
        offset: w[1],
        // TODO: more accurate offset? might need to fork ansi-sequence-parser
        color: o,
        bgColor: s,
        fontStyle: e
      };
    })
  );
}
function VE(B) {
  const A = B.match(/#([0-9a-f]{3})([0-9a-f]{3})?([0-9a-f]{2})?/);
  if (A)
    if (A[3]) {
      const g = Math.round(Number.parseInt(A[3], 16) / 2).toString(16).padStart(2, "0");
      return `#${A[1]}${A[2]}${g}`;
    } else return A[2] ? `#${A[1]}${A[2]}80` : `#${Array.from(A[1]).map((g) => `${g}${g}`).join("")}80`;
  const Q = B.match(/var\((--[\w-]+-ansi-[\w-]+)\)/);
  return Q ? `var(${Q[1]}-dim)` : B;
}
function AB(B, A, Q = {}) {
  const {
    lang: g = "text",
    theme: E = B.getLoadedThemes()[0]
  } = Q;
  if (_A(g) || $A(E))
    return HA(A).map((D) => [{ content: D[0], offset: D[1] }]);
  const { theme: C, colorMap: I } = B.setTheme(E);
  if (g === "ansi")
    return vE(C, A, Q);
  const w = B.getLanguage(g);
  if (Q.grammarState) {
    if (Q.grammarState.lang !== w.name)
      throw new b(`Grammar state language "${Q.grammarState.lang}" does not match highlight language "${w.name}"`);
    if (!Q.grammarState.themes.includes(C.name))
      throw new b(`Grammar state themes "${Q.grammarState.themes}" do not contain highlight theme "${C.name}"`);
  }
  return jE(A, w, C, I, Q);
}
function zE(...B) {
  if (B.length === 2)
    return gA(B[1]);
  const [A, Q, g = {}] = B, {
    lang: E = "text",
    theme: C = A.getLoadedThemes()[0]
  } = g;
  if (_A(E) || $A(C))
    throw new b("Plain language does not have grammar state");
  if (E === "ansi")
    throw new b("ANSI language does not have grammar state");
  const { theme: I, colorMap: w } = A.setTheme(C), D = A.getLanguage(E);
  return new z(
    GA(Q, D, I, w, g).stateStack,
    D.name,
    I.name
  );
}
function jE(B, A, Q, g, E) {
  const C = GA(B, A, Q, g, E), I = new z(
    GA(B, A, Q, g, E).stateStack,
    A.name,
    Q.name
  );
  return RA(C.tokens, I), C.tokens;
}
function GA(B, A, Q, g, E) {
  const C = hA(Q, E), {
    tokenizeMaxLineLength: I = 0,
    tokenizeTimeLimit: w = 500
  } = E, D = HA(B);
  let o = E.grammarState ? OE(E.grammarState, Q.name) ?? mA : E.grammarContextCode != null ? GA(
    E.grammarContextCode,
    A,
    Q,
    g,
    {
      ...E,
      grammarState: void 0,
      grammarContextCode: void 0
    }
  ).stateStack : mA, s = [];
  const e = [];
  for (let n = 0, i = D.length; n < i; n++) {
    const [t, N] = D[n];
    if (t === "") {
      s = [], e.push([]);
      continue;
    }
    if (I > 0 && t.length >= I) {
      s = [], e.push([{
        content: t,
        offset: N,
        color: "",
        fontStyle: 0
      }]);
      continue;
    }
    let M, h, a;
    E.includeExplanation && (M = A.tokenizeLine(t, o), h = M.tokens, a = 0);
    const F = A.tokenizeLine2(t, o, w), R = F.tokens.length / 2;
    for (let L = 0; L < R; L++) {
      const u = F.tokens[2 * L], O = L + 1 < R ? F.tokens[2 * L + 2] : t.length;
      if (u === O)
        continue;
      const QB = F.tokens[2 * L + 1], oQ = q(
        g[v.getForeground(QB)],
        C
      ), sQ = v.getFontStyle(QB), kA = {
        content: t.substring(u, O),
        offset: N + u,
        color: oQ,
        fontStyle: sQ
      };
      if (E.includeExplanation) {
        const gB = [];
        if (E.includeExplanation !== "scopeName")
          for (const f of Q.settings) {
            let W;
            switch (typeof f.scope) {
              case "string":
                W = f.scope.split(/,/).map((KA) => KA.trim());
                break;
              case "object":
                W = f.scope;
                break;
              default:
                continue;
            }
            gB.push({
              settings: f,
              selectors: W.map((KA) => KA.split(/ /))
            });
          }
        kA.explanation = [];
        let EB = 0;
        for (; u + EB < O; ) {
          const f = h[a], W = t.substring(
            f.startIndex,
            f.endIndex
          );
          EB += W.length, kA.explanation.push({
            content: W,
            scopes: E.includeExplanation === "scopeName" ? _E(
              f.scopes
            ) : $E(
              gB,
              f.scopes
            )
          }), a += 1;
        }
      }
      s.push(kA);
    }
    e.push(s), s = [], o = F.ruleStack;
  }
  return {
    tokens: e,
    stateStack: o
  };
}
function _E(B) {
  return B.map((A) => ({ scopeName: A }));
}
function $E(B, A) {
  const Q = [];
  for (let g = 0, E = A.length; g < E; g++) {
    const C = A[g];
    Q[g] = {
      scopeName: C,
      themeMatches: BC(B, C, A.slice(0, g))
    };
  }
  return Q;
}
function LB(B, A) {
  return B === A || A.substring(0, B.length) === B && A[B.length] === ".";
}
function AC(B, A, Q) {
  if (!LB(B[B.length - 1], A))
    return !1;
  let g = B.length - 2, E = Q.length - 1;
  for (; g >= 0 && E >= 0; )
    LB(B[g], Q[E]) && (g -= 1), E -= 1;
  return g === -1;
}
function BC(B, A, Q) {
  const g = [];
  for (const { selectors: E, settings: C } of B)
    for (const I of E)
      if (AC(I, A, Q)) {
        g.push(C);
        break;
      }
  return g;
}
function IQ(B, A, Q) {
  const g = Object.entries(Q.themes).filter((D) => D[1]).map((D) => ({ color: D[0], theme: D[1] })), E = g.map((D) => {
    const o = AB(B, A, {
      ...Q,
      theme: D.theme
    }), s = gA(o), e = typeof D.theme == "string" ? D.theme : D.theme.name;
    return {
      tokens: o,
      state: s,
      theme: e
    };
  }), C = QC(
    ...E.map((D) => D.tokens)
  ), I = C[0].map(
    (D, o) => D.map((s, e) => {
      const n = {
        content: s.content,
        variants: {},
        offset: s.offset
      };
      return "includeExplanation" in Q && Q.includeExplanation && (n.explanation = s.explanation), C.forEach((i, t) => {
        const {
          content: N,
          explanation: M,
          offset: h,
          ...a
        } = i[o][e];
        n.variants[g[t].color] = a;
      }), n;
    })
  ), w = E[0].state ? new z(
    Object.fromEntries(E.map((D) => [D.theme, D.state?.getInternalStack(D.theme)])),
    E[0].state.lang
  ) : void 0;
  return w && RA(I, w), I;
}
function QC(...B) {
  const A = B.map(() => []), Q = B.length;
  for (let g = 0; g < B[0].length; g++) {
    const E = B.map((D) => D[g]), C = A.map(() => []);
    A.forEach((D, o) => D.push(C[o]));
    const I = E.map(() => 0), w = E.map((D) => D[0]);
    for (; w.every((D) => D); ) {
      const D = Math.min(...w.map((o) => o.content.length));
      for (let o = 0; o < Q; o++) {
        const s = w[o];
        s.content.length === D ? (C[o].push(s), I[o] += 1, w[o] = E[o][I[o]]) : (C[o].push({
          ...s,
          content: s.content.slice(0, D)
        }), w[o] = {
          ...s,
          content: s.content.slice(D),
          offset: s.offset + D
        });
      }
    }
  }
  return A;
}
function NA(B, A, Q) {
  let g, E, C, I, w, D;
  if ("themes" in Q) {
    const {
      defaultColor: o = "light",
      cssVariablePrefix: s = "--shiki-"
    } = Q, e = Object.entries(Q.themes).filter((M) => M[1]).map((M) => ({ color: M[0], theme: M[1] })).sort((M, h) => M.color === o ? -1 : h.color === o ? 1 : 0);
    if (e.length === 0)
      throw new b("`themes` option must not be empty");
    const n = IQ(
      B,
      A,
      Q
    );
    if (D = gA(n), o && !e.find((M) => M.color === o))
      throw new b(`\`themes\` option must contain the defaultColor key \`${o}\``);
    const i = e.map((M) => B.getTheme(M.theme)), t = e.map((M) => M.color);
    C = n.map((M) => M.map((h) => gC(h, t, s, o))), D && RA(C, D);
    const N = e.map((M) => hA(M.theme, Q));
    E = e.map((M, h) => (h === 0 && o ? "" : `${s + M.color}:`) + (q(i[h].fg, N[h]) || "inherit")).join(";"), g = e.map((M, h) => (h === 0 && o ? "" : `${s + M.color}-bg:`) + (q(i[h].bg, N[h]) || "inherit")).join(";"), I = `shiki-themes ${i.map((M) => M.name).join(" ")}`, w = o ? void 0 : [E, g].join(";");
  } else if ("theme" in Q) {
    const o = hA(Q.theme, Q);
    C = AB(
      B,
      A,
      Q
    );
    const s = B.getTheme(Q.theme);
    g = q(s.bg, o), E = q(s.fg, o), I = s.name, D = gA(C);
  } else
    throw new b("Invalid options, either `theme` or `themes` must be provided");
  return {
    tokens: C,
    fg: E,
    bg: g,
    themeName: I,
    rootStyle: w,
    grammarState: D
  };
}
function gC(B, A, Q, g) {
  const E = {
    content: B.content,
    explanation: B.explanation,
    offset: B.offset
  }, C = A.map((D) => gQ(B.variants[D])), I = new Set(C.flatMap((D) => Object.keys(D))), w = {};
  return C.forEach((D, o) => {
    for (const s of I) {
      const e = D[s] || "inherit";
      if (o === 0 && g)
        w[s] = e;
      else {
        const n = s === "color" ? "" : s === "background-color" ? "-bg" : `-${s}`, i = Q + A[o] + (s === "color" ? "" : n);
        w[i] = e;
      }
    }
  }), E.htmlStyle = w, E;
}
function rA(B, A, Q, g = {
  meta: {},
  options: Q,
  codeToHast: (E, C) => rA(B, E, C),
  codeToTokens: (E, C) => NA(B, E, C)
}) {
  let E = A;
  for (const i of FA(Q))
    E = i.preprocess?.call(g, E, Q) || E;
  let {
    tokens: C,
    fg: I,
    bg: w,
    themeName: D,
    rootStyle: o,
    grammarState: s
  } = NA(B, E, Q);
  const {
    mergeWhitespaces: e = !0
  } = Q;
  e === !0 ? C = CC(C) : e === "never" && (C = IC(C));
  const n = {
    ...g,
    get source() {
      return E;
    }
  };
  for (const i of FA(Q))
    C = i.tokens?.call(n, C) || C;
  return EC(
    C,
    {
      ...Q,
      fg: I,
      bg: w,
      themeName: D,
      rootStyle: o
    },
    n,
    s
  );
}
function EC(B, A, Q, g = gA(B)) {
  const E = FA(A), C = [], I = {
    type: "root",
    children: []
  }, {
    structure: w = "classic",
    tabindex: D = "0"
  } = A;
  let o = {
    type: "element",
    tagName: "pre",
    properties: {
      class: `shiki ${A.themeName || ""}`,
      style: A.rootStyle || `background-color:${A.bg};color:${A.fg}`,
      ...D !== !1 && D != null ? {
        tabindex: D.toString()
      } : {},
      ...Object.fromEntries(
        Array.from(
          Object.entries(A.meta || {})
        ).filter(([t]) => !t.startsWith("_"))
      )
    },
    children: []
  }, s = {
    type: "element",
    tagName: "code",
    properties: {},
    children: C
  };
  const e = [], n = {
    ...Q,
    structure: w,
    addClassToHast: BQ,
    get source() {
      return Q.source;
    },
    get tokens() {
      return B;
    },
    get options() {
      return A;
    },
    get root() {
      return I;
    },
    get pre() {
      return o;
    },
    get code() {
      return s;
    },
    get lines() {
      return e;
    }
  };
  if (B.forEach((t, N) => {
    N && (w === "inline" ? I.children.push({ type: "element", tagName: "br", properties: {}, children: [] }) : w === "classic" && C.push({ type: "text", value: `
` }));
    let M = {
      type: "element",
      tagName: "span",
      properties: { class: "line" },
      children: []
    }, h = 0;
    for (const a of t) {
      let F = {
        type: "element",
        tagName: "span",
        properties: {
          ...a.htmlAttrs
        },
        children: [{ type: "text", value: a.content }]
      };
      a.htmlStyle;
      const R = fE(a.htmlStyle || gQ(a));
      R && (F.properties.style = R);
      for (const L of E)
        F = L?.span?.call(n, F, N + 1, h, M, a) || F;
      w === "inline" ? I.children.push(F) : w === "classic" && M.children.push(F), h += a.content.length;
    }
    if (w === "classic") {
      for (const a of E)
        M = a?.line?.call(n, M, N + 1) || M;
      e.push(M), C.push(M);
    }
  }), w === "classic") {
    for (const t of E)
      s = t?.code?.call(n, s) || s;
    o.children.push(s);
    for (const t of E)
      o = t?.pre?.call(n, o) || o;
    I.children.push(o);
  }
  let i = I;
  for (const t of E)
    i = t?.root?.call(n, i) || i;
  return g && RA(i, g), i;
}
function CC(B) {
  return B.map((A) => {
    const Q = [];
    let g = "", E = 0;
    return A.forEach((C, I) => {
      const D = !(C.fontStyle && C.fontStyle & d.Underline);
      D && C.content.match(/^\s+$/) && A[I + 1] ? (E || (E = C.offset), g += C.content) : g ? (D ? Q.push({
        ...C,
        offset: E,
        content: g + C.content
      }) : Q.push(
        {
          content: g,
          offset: E
        },
        C
      ), E = 0, g = "") : Q.push(C);
    }), Q;
  });
}
function IC(B) {
  return B.map((A) => A.flatMap((Q) => {
    if (Q.content.match(/^\s+$/))
      return Q;
    const g = Q.content.match(/^(\s*)(.*?)(\s*)$/);
    if (!g)
      return Q;
    const [, E, C, I] = g;
    if (!E && !I)
      return Q;
    const w = [{
      ...Q,
      offset: Q.offset + E.length,
      content: C
    }];
    return E && w.unshift({
      content: E,
      offset: Q.offset
    }), I && w.push({
      content: I,
      offset: Q.offset + E.length + C.length
    }), w;
  }));
}
function DC(B, A, Q) {
  const g = {
    meta: {},
    options: Q,
    codeToHast: (C, I) => rA(B, C, I),
    codeToTokens: (C, I) => NA(B, C, I)
  };
  let E = YE(rA(B, A, Q, g));
  for (const C of FA(Q))
    E = C.postprocess?.call(g, E, Q) || E;
  return E;
}
const HB = { light: "#333333", dark: "#bbbbbb" }, RB = { light: "#fffffe", dark: "#1e1e1e" }, kB = "__shiki_resolved";
function BB(B) {
  if (B?.[kB])
    return B;
  const A = {
    ...B
  };
  A.tokenColors && !A.settings && (A.settings = A.tokenColors, delete A.tokenColors), A.type ||= "dark", A.colorReplacements = { ...A.colorReplacements }, A.settings ||= [];
  let { bg: Q, fg: g } = A;
  if (!Q || !g) {
    const w = A.settings ? A.settings.find((D) => !D.name && !D.scope) : void 0;
    w?.settings?.foreground && (g = w.settings.foreground), w?.settings?.background && (Q = w.settings.background), !g && A?.colors?.["editor.foreground"] && (g = A.colors["editor.foreground"]), !Q && A?.colors?.["editor.background"] && (Q = A.colors["editor.background"]), g || (g = A.type === "light" ? HB.light : HB.dark), Q || (Q = A.type === "light" ? RB.light : RB.dark), A.fg = g, A.bg = Q;
  }
  A.settings[0] && A.settings[0].settings && !A.settings[0].scope || A.settings.unshift({
    settings: {
      foreground: A.fg,
      background: A.bg
    }
  });
  let E = 0;
  const C = /* @__PURE__ */ new Map();
  function I(w) {
    if (C.has(w))
      return C.get(w);
    E += 1;
    const D = `#${E.toString(16).padStart(8, "0").toLowerCase()}`;
    return A.colorReplacements?.[`#${D}`] ? I(w) : (C.set(w, D), D);
  }
  A.settings = A.settings.map((w) => {
    const D = w.settings?.foreground && !w.settings.foreground.startsWith("#"), o = w.settings?.background && !w.settings.background.startsWith("#");
    if (!D && !o)
      return w;
    const s = {
      ...w,
      settings: {
        ...w.settings
      }
    };
    if (D) {
      const e = I(w.settings.foreground);
      A.colorReplacements[e] = w.settings.foreground, s.settings.foreground = e;
    }
    if (o) {
      const e = I(w.settings.background);
      A.colorReplacements[e] = w.settings.background, s.settings.background = e;
    }
    return s;
  });
  for (const w of Object.keys(A.colors || {}))
    if ((w === "editor.foreground" || w === "editor.background" || w.startsWith("terminal.ansi")) && !A.colors[w]?.startsWith("#")) {
      const D = I(A.colors[w]);
      A.colorReplacements[D] = A.colors[w], A.colors[w] = D;
    }
  return Object.defineProperty(A, kB, {
    enumerable: !1,
    writable: !1,
    value: !0
  }), A;
}
async function DQ(B) {
  return Array.from(new Set((await Promise.all(
    B.filter((A) => !JE(A)).map(async (A) => await QQ(A).then((Q) => Array.isArray(Q) ? Q : [Q]))
  )).flat()));
}
async function wQ(B) {
  return (await Promise.all(
    B.map(
      async (Q) => uE(Q) ? null : BB(await QQ(Q))
    )
  )).filter((Q) => !!Q);
}
class wC extends Mg {
  constructor(A, Q, g, E = {}) {
    super(A), this._resolver = A, this._themes = Q, this._langs = g, this._alias = E, this._themes.map((C) => this.loadTheme(C)), this.loadLanguages(this._langs);
  }
  _resolvedThemes = /* @__PURE__ */ new Map();
  _resolvedGrammars = /* @__PURE__ */ new Map();
  _langMap = /* @__PURE__ */ new Map();
  _langGraph = /* @__PURE__ */ new Map();
  _textmateThemeCache = /* @__PURE__ */ new WeakMap();
  _loadedThemesCache = null;
  _loadedLanguagesCache = null;
  getTheme(A) {
    return typeof A == "string" ? this._resolvedThemes.get(A) : this.loadTheme(A);
  }
  loadTheme(A) {
    const Q = BB(A);
    return Q.name && (this._resolvedThemes.set(Q.name, Q), this._loadedThemesCache = null), Q;
  }
  getLoadedThemes() {
    return this._loadedThemesCache || (this._loadedThemesCache = [...this._resolvedThemes.keys()]), this._loadedThemesCache;
  }
  // Override and re-implement this method to cache the textmate themes as `TextMateTheme.createFromRawTheme`
  // is expensive. Themes can switch often especially for dual-theme support.
  //
  // The parent class also accepts `colorMap` as the second parameter, but since we don't use that,
  // we omit here so it's easier to cache the themes.
  setTheme(A) {
    let Q = this._textmateThemeCache.get(A);
    Q || (Q = tA.createFromRawTheme(A), this._textmateThemeCache.set(A, Q)), this._syncRegistry.setTheme(Q);
  }
  getGrammar(A) {
    if (this._alias[A]) {
      const Q = /* @__PURE__ */ new Set([A]);
      for (; this._alias[A]; ) {
        if (A = this._alias[A], Q.has(A))
          throw new l(`Circular alias \`${Array.from(Q).join(" -> ")} -> ${A}\``);
        Q.add(A);
      }
    }
    return this._resolvedGrammars.get(A);
  }
  loadLanguage(A) {
    if (this.getGrammar(A.name))
      return;
    const Q = new Set(
      [...this._langMap.values()].filter((C) => C.embeddedLangsLazy?.includes(A.name))
    );
    this._resolver.addLanguage(A);
    const g = {
      balancedBracketSelectors: A.balancedBracketSelectors || ["*"],
      unbalancedBracketSelectors: A.unbalancedBracketSelectors || []
    };
    this._syncRegistry._rawGrammars.set(A.scopeName, A);
    const E = this.loadGrammarWithConfiguration(A.scopeName, 1, g);
    if (E.name = A.name, this._resolvedGrammars.set(A.name, E), A.aliases && A.aliases.forEach((C) => {
      this._alias[C] = A.name;
    }), this._loadedLanguagesCache = null, Q.size)
      for (const C of Q)
        this._resolvedGrammars.delete(C.name), this._loadedLanguagesCache = null, this._syncRegistry?._injectionGrammars?.delete(C.scopeName), this._syncRegistry?._grammars?.delete(C.scopeName), this.loadLanguage(this._langMap.get(C.name));
  }
  dispose() {
    super.dispose(), this._resolvedThemes.clear(), this._resolvedGrammars.clear(), this._langMap.clear(), this._langGraph.clear(), this._loadedThemesCache = null;
  }
  loadLanguages(A) {
    for (const E of A)
      this.resolveEmbeddedLanguages(E);
    const Q = Array.from(this._langGraph.entries()), g = Q.filter(([E, C]) => !C);
    if (g.length) {
      const E = Q.filter(([C, I]) => I && I.embeddedLangs?.some((w) => g.map(([D]) => D).includes(w))).filter((C) => !g.includes(C));
      throw new l(`Missing languages ${g.map(([C]) => `\`${C}\``).join(", ")}, required by ${E.map(([C]) => `\`${C}\``).join(", ")}`);
    }
    for (const [E, C] of Q)
      this._resolver.addLanguage(C);
    for (const [E, C] of Q)
      this.loadLanguage(C);
  }
  getLoadedLanguages() {
    return this._loadedLanguagesCache || (this._loadedLanguagesCache = [
      .../* @__PURE__ */ new Set([...this._resolvedGrammars.keys(), ...Object.keys(this._alias)])
    ]), this._loadedLanguagesCache;
  }
  resolveEmbeddedLanguages(A) {
    if (this._langMap.set(A.name, A), this._langGraph.set(A.name, A), A.embeddedLangs)
      for (const Q of A.embeddedLangs)
        this._langGraph.set(Q, this._langMap.get(Q));
  }
}
class oC {
  _langs = /* @__PURE__ */ new Map();
  _scopeToLang = /* @__PURE__ */ new Map();
  _injections = /* @__PURE__ */ new Map();
  _onigLib;
  constructor(A, Q) {
    this._onigLib = {
      createOnigScanner: (g) => A.createScanner(g),
      createOnigString: (g) => A.createString(g)
    }, Q.forEach((g) => this.addLanguage(g));
  }
  get onigLib() {
    return this._onigLib;
  }
  getLangRegistration(A) {
    return this._langs.get(A);
  }
  loadGrammar(A) {
    return this._scopeToLang.get(A);
  }
  addLanguage(A) {
    this._langs.set(A.name, A), A.aliases && A.aliases.forEach((Q) => {
      this._langs.set(Q, A);
    }), this._scopeToLang.set(A.scopeName, A), A.injectTo && A.injectTo.forEach((Q) => {
      this._injections.get(Q) || this._injections.set(Q, []), this._injections.get(Q).push(A.scopeName);
    });
  }
  getInjections(A) {
    const Q = A.split(".");
    let g = [];
    for (let E = 1; E <= Q.length; E++) {
      const C = Q.slice(0, E).join(".");
      g = [...g, ...this._injections.get(C) || []];
    }
    return g;
  }
}
let j = 0;
function sC(B) {
  j += 1, B.warnings !== !1 && j >= 10 && j % 10 === 0 && console.warn(`[Shiki] ${j} instances have been created. Shiki is supposed to be used as a singleton, consider refactoring your code to cache your highlighter instance; Or call \`highlighter.dispose()\` to release unused instances.`);
  let A = !1;
  if (!B.engine)
    throw new l("`engine` option is required for synchronous mode");
  const Q = (B.langs || []).flat(1), g = (B.themes || []).flat(1).map(BB), E = new oC(B.engine, Q), C = new wC(E, g, Q, B.langAlias);
  let I;
  function w(a) {
    M();
    const F = C.getGrammar(typeof a == "string" ? a : a.name);
    if (!F)
      throw new l(`Language \`${a}\` not found, you may need to load it first`);
    return F;
  }
  function D(a) {
    if (a === "none")
      return { bg: "", fg: "", name: "none", settings: [], type: "dark" };
    M();
    const F = C.getTheme(a);
    if (!F)
      throw new l(`Theme \`${a}\` not found, you may need to load it first`);
    return F;
  }
  function o(a) {
    M();
    const F = D(a);
    I !== a && (C.setTheme(F), I = a);
    const R = C.getColorMap();
    return {
      theme: F,
      colorMap: R
    };
  }
  function s() {
    return M(), C.getLoadedThemes();
  }
  function e() {
    return M(), C.getLoadedLanguages();
  }
  function n(...a) {
    M(), C.loadLanguages(a.flat(1));
  }
  async function i(...a) {
    return n(await DQ(a));
  }
  function t(...a) {
    M();
    for (const F of a.flat(1))
      C.loadTheme(F);
  }
  async function N(...a) {
    return M(), t(await wQ(a));
  }
  function M() {
    if (A)
      throw new l("Shiki instance has been disposed");
  }
  function h() {
    A || (A = !0, C.dispose(), j -= 1);
  }
  return {
    setTheme: o,
    getTheme: D,
    getLanguage: w,
    getLoadedThemes: s,
    getLoadedLanguages: e,
    loadLanguage: i,
    loadLanguageSync: n,
    loadTheme: N,
    loadThemeSync: t,
    dispose: h,
    [Symbol.dispose]: h
  };
}
async function eC(B = {}) {
  B.loadWasm;
  const [
    A,
    Q,
    g
  ] = await Promise.all([
    wQ(B.themes || []),
    DQ(B.langs || []),
    B.engine || lQ(B.loadWasm || YQ())
  ]);
  return sC({
    ...B,
    themes: A,
    langs: Q,
    engine: g
  });
}
async function GC(B = {}) {
  const A = await eC(B);
  return {
    getLastGrammarState: (...Q) => zE(A, ...Q),
    codeToTokensBase: (Q, g) => AB(A, Q, g),
    codeToTokensWithThemes: (Q, g) => IQ(A, Q, g),
    codeToTokens: (Q, g) => NA(A, Q, g),
    codeToHast: (Q, g) => rA(A, Q, g),
    codeToHtml: (Q, g) => DC(A, Q, g),
    ...A,
    getInternalContext: () => A
  };
}
var iC = Uint8Array.from(atob(""), (B) => B.charCodeAt(0));
const tC = iC, NC = async (B) => WebAssembly.instantiate(tC, B).then((A) => A.instance.exports);
export {
  hC as a,
  GC as c,
  Lg as f,
  NC as g,
  kg as h,
  JE as i,
  bA as n,
  FC as p,
  VB as s,
  YE as t
};
//# sourceMappingURL=shiki_CE0IyDxo.js.map
