...
async function ue(o, r = () => {
}) {
  r("markdownToVue started");
  const { content: t, data: a } = oe(o);
  r("matter extracted");
  const n = A(t, {
  ...
  const s = await J().data("settings", { fragment: !0 }).use(M).use(q).use(G, await ce(), {
    theme: "github-dark",
    colorReplacements: {
      "#24292e": "#252423"
    }
...
async function we(o, r) {
  ...
  try {
    let { vueTemplate: n, frontMatter: i } = await ue(o, a);
    t.debuggingInfo.vueTemplate = n, t.compiled.frontMatter = i;
    {
      const u = await ge.generate(n);
      u.css && (n += `
